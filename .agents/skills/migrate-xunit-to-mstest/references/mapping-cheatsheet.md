...
>
> **Conditional skips:** preserve the source gating predicate and evaluation point; see Section 3.9 for runtime skips and condition-attribute compatibility.
...
| `Assert.Collection(items, e1 => ..., e2 => ...)`                    | **Manual** -- use the single-pass inspector mapping in Section 3.11.                                                                                                                                                                                                                                         |
...
>
> Preserve the original runtime predicate and evaluation point. Use a condition attribute only when the selected MSTest release provides it and its predicate and discovery-time behavior preserve the source test's behavior. Runtime `Assert.Inconclusive` outcomes can be reported differently from xUnit skips; verify the selected runner's result classification.

| xUnit                                                                      | MSTest                                                                              |
| -------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| `[Fact(Skip = "reason")]`                                                  | `[TestMethod]` + `[Ignore("reason")]`                                               |
| `Assert.Skip("reason")` (xUnit v3)                                         | `Assert.Inconclusive("reason")`                                                     |
| `Assert.SkipWhen(condition, "reason")` (xUnit v3)                          | `if (condition) Assert.Inconclusive("reason");` at the original evaluation point    |
| `Assert.SkipUnless(condition, "reason")` (xUnit v3)                        | `if (!condition) Assert.Inconclusive("reason");` at the original evaluation point   |
| `Assert.SkipUnless(OperatingSystem.IsWindows(), "...")`                    | `if (!OperatingSystem.IsWindows()) Assert.Inconclusive("...");`                     |
| `Assert.SkipWhen(Environment.GetEnvironmentVariable("CI") != null, "...")` | `if (Environment.GetEnvironmentVariable("CI") != null) Assert.Inconclusive("...");` |
...
| ------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Assert.Collection(items, e1Inspector, e2Inspector, ...)`           | Traverse once, invoking each inspector on its corresponding element in order, and verify the final count. Preserve the source interleaving of enumeration and inspection; do not count or materialize the input first. |
...
| xUnit                                                                                    | MSTest                                                                                                                                                                               |
| ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `[assembly: AssemblyFixture(typeof(T))]` (xUnit v3); v2 has no built-in assembly fixture | `[AssemblyInitialize]` and `[AssemblyCleanup]` in a `[TestClass]`; preserve assembly-wide sharing, initialization, and cleanup, including asynchronous lifecycle methods where used. |
...
| xUnit + `[CollectionBehavior(MaxParallelThreads = N)]`                                                 | For supported nonnegative `N`, `[assembly: Parallelize(Workers = N, Scope = ExecutionScope.ClassLevel)]`; `0` uses the processor count. xUnit's unlimited `-1` and other special runner values require explicit runner-aware adaptation; do not copy them into `Workers` or silently cap concurrency. |
...
| `"maxParallelThreads": N`                                    | For supported nonnegative `N`, `[assembly: Parallelize(Workers = N, Scope = ExecutionScope.ClassLevel)]`; `0` uses the processor count. xUnit's unlimited `-1` and other special runner values require explicit runner-aware adaptation; do not copy them into `Workers` or silently cap concurrency. |
...
| `[assembly: AssemblyFixture(typeof(T))]` (xUnit v3)              | Replace using the assembly-wide fixture lifecycle mapping in Section 4.                                                                                                                                                                                    |
| `[assembly: CaptureConsole]` (xUnit v3)                          | Remove the source attribute. MSTest 4.1's adapter captures stdout/stderr by default through `CaptureDebugTraces`; preserve explicit source capture options using supported runner settings and report unsupported differences.                             |
...
| `Xunit.StaFact` (`[StaFact]`, `[WpfFact]`)                             | For `[StaFact]`, use MSTest 4.1+ `[STATestMethod]` with Windows-only gating preserved and its default `UseSTASynchronizationContext = false`. `[WpfFact]` also needs a WPF dispatcher synchronization context: adapt manually; generic STA alone is insufficient.               |
| `Xunit.Priority` (`[TestCaseOrderer]`)                                 | MSTest ordering is different -- flag for manual                                                                                                                                                                                                                                 |
| `Verify.Xunit`                                                         | Use `Verify.MSTest` and the selected version's documented integration: `UsesVerify` source-generator opt-in with partial test classes, or `VerifyBase`. Preserve an existing required base class and verify snapshot naming and behavior; a package swap alone is insufficient. |
