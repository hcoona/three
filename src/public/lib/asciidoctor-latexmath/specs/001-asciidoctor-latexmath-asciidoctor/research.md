...
> This is historical research for the imported design baseline. In particular, its earlier cache-key proposal includes tool identities that the later P5 and specification exclude. Preserve the rejected proposal as rationale; use the specification and retained rendering contract for that decision.
