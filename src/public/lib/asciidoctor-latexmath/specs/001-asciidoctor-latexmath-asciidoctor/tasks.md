...
> This retained traceability input is consumed by scripts/governance_audit.rb and its existing test. Legacy task IDs and requirement references remain intact. Checkboxes, phase labels and next-step instructions describe the historical plan; they neither authorize work nor certify current test results.
...
- [x] T007 [P] Renderer pipeline contract spec: `spec/rendering/pipeline_contract_spec.rb` (stage order 固定; 变更需版本 bump; timeout 占位) (P5) _不再测试 pipeline_signature 字段_。
...
_Based on Constitution v3.1.0 – see `docs/architecture/rendering-and-quality-contract.md`_
