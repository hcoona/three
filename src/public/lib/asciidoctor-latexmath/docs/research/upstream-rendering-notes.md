...
> Retained imported design input, with its original evidence level and language.
> Source: [original record](https://github.com/hcoona/three/blob/b673ee27aab8553a4ee259bcaca87a197178463c/src/public/lib/asciidoctor-latexmath/LEARN.md). This is not a new support promise
> or an instruction to resume an old task. See the [project record portal](../README.md)
> for requirements, constraints, contracts and current user documentation.

The [original learning input](processor-selection-input.txt) preserves the complete
supplied task and six human/ChatGPT exchanges that informed the processor,
parsing-time and attribute-snapshot discussion in sections 5–6 below. Its bytes
are unchanged from [the accepted source](https://github.com/hcoona/three/blob/b673ee27aab8553a4ee259bcaca87a197178463c/src/public/lib/asciidoctor-latexmath/.github/prompts/learn.prompt.md).
It is static research input, not a reusable prompt or permission to resume the
old learning task. Human questions and assistant advice retain their separate
attribution; linked sources and API examples in that conversation have not been
revalidated by this migration. Suggestions involving block macros, TreeProcessor
or additional preprocessing do not override [P1–P5](../architecture/rendering-and-quality-contract.md)
or the [current requirements](../../specs/001-asciidoctor-latexmath-asciidoctor/spec.md).
...
- **后续行动**: 在 `docs/architecture/overview.md` 中固化整体架构、组件职责、PlantUML 类图；实现阶段遵循该文档拆分模块。
...
- **产出**: `docs/architecture/overview.md` 更新后的类图、时序图与正文说明，改写 Renderer 抽象以匹配组合式渲染策略。
...
- **阅读材料**:
    ...
    - `docs/research/latexmath-attribute-input.md`（Lines 1-180）：梳理 `latexmath-format`、`latexmath-cache`、`latexmath-preamble`、`latexmath-png-tool` 等全量属性及块级 `nocache`/`keep-artifacts` 选项。
- **关键结论**:
...
- **后续行动**:
    - 在 `docs/architecture/overview.md` 中替换旧类图与描述，记录新的管线层次、缓存与元数据需求。
