...
> Source: [original record](https://github.com/hcoona/three/blob/b673ee27aab8553a4ee259bcaca87a197178463c/src/public/lib/asciidoctor-latexmath/AsciidoctorLatexmathAttributes.md). This is not a new support promise
...
本文基于现有文档（`README.md`、`docs/architecture/overview.md` 等）以及 asciidoctor-diagram 的属性设计，按照渲染流水线对 asciidoctor-latexmath 计划支持的配置开关进行分组：
...
| ----------- | --------------------------------------------- | ----------------------------------- | -------------- | ------------------------------------------------------------------------------------------------- |
| 全局控制    | 解析 Asciidoctor 属性、决定缓存策略与输出格式 | _(无)_                              | _(配置预处理)_ | `stem`, `latexmath-format`, `latexmath-cache`, `latexmath-cachedir`                               |
| 阶段一      | 生成临时 `.tex` 文档                          | _(模板生成)_                        | `.tex`         | `latexmath-preamble`, `latexmath-fontsize`, `latexmath-keep-artifacts`, `latexmath-artifacts-dir` |
| 阶段二      | 调用 LaTeX 引擎编译                           | `pdflatex`                          | `.pdf`         | `pdflatex`（可指向 `xelatex`/`lualatex`/`tectonic` 等）                                           |
| 阶段三（A） | 直接输出 PDF                                  | _(跳过额外命令)_                    | `.pdf`         | `latexmath-format=pdf`、块/内联 `format=pdf`                                                      |
...
| -------------------- | --------------------------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------- |
| `stem`               | `:stem: latexmath` 或 `:stem: tex`      | _(无)_                                                     | _(未设置)_                                                                                             | 启用 Asciidoctor 的 STEM 管线，让 latexmath 扩展接管块与内联渲染。    |
...
| `latexmath-cachedir` | `:latexmath-cachedir: .cache/latexmath` | `[latexmath, cachedir=...]` _(可选)_                       | `<outdir>/.asciidoctor/latexmath`（若设置 `:imagesdir:` 则回退 `<outdir>/<imagesdir>`，见规范 FR-037） | 指定缓存目录位置。相对路径基于文档目录解析。                          |
...
| -------------------------- | ----------------------------------------- | -------------------------------------------------------- | ---------------------------- | -------------------------------------------------------- | --------------------------- |
| `latexmath-preamble`       | `:latexmath-preamble: \usepackage{bm}`    | `[latexmath, preamble="\\usepackage{bm}"]`               | _(空)_                       | 为生成的独立 `.tex` 文档追加前导宏包或自定义命令。       | 影响 `.tex` 输入            |
...
| `latexmath-png-tool` | `:latexmath-png-tool: magick` | `[latexmath, png-tool=pdftoppm]` / `latexmath:[...,png-tool=gs]` | _(自动探测)_ | 强制选择 PNG 渲染工具，默认为依次探测 `pdftoppm`、`magick`、`gs`。 | Poppler、ImageMagick、Ghostscript |
