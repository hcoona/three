...
one—is rendered by the imported local build from this repository branch. Hexo
...
use the clean-checkout build and local `file:` workflow in
...
This repository depends on `hexo-renderer-asciidoc` via a local `file:`
dependency. From a clean clone, first follow the exact build/install/generate
sequence in `examples/hexo-site/README.md`. After that, you can edit the demo
content while `pnpm dev` runs inside `examples/hexo-site`. After changing the
renderer, rebuild it, reinstall the example's local dependency, and restart
Hexo.
...
To rebuild the parent package, refresh the example dependency, and regenerate
the site, stay at the repository root and run:
