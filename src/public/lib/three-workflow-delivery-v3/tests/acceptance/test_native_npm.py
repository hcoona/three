...
No publication API or process outcome is simulated here. These tests pin
...
def test_canonical_state_has_a_closed_deterministic_semantic_shape():
    ...
    expected: dict[str, JsonValue] = {
        "schema": "workflow-delivery-v3/native-npm-state/v2",
        "control": {
            "container_id": 101,
...
def test_content_membership_must_be_bound_to_actual_active_inventory():
    """Retain dangling tags without inferring any administrative history."""
    dangling = replace(_baseline(), tags=(*_baseline().tags, (TAG, "2.0.0")))
    assert dict(dangling.tags)[TAG] == "2.0.0"
    assert "2.0.0" not in dangling.active_versions
    with pytest.raises(
...
def test_inventories_require_immutable_sorted_unique_names():
