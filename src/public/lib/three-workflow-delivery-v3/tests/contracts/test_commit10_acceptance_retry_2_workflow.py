"""Contracts for the temporary destination-acceptance retry-2 workflow."""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("confirm", "I_ACCEPT_DISPOSABLE_GITHUB_PACKAGES_PROBES_RETRY_2"),
...
@pytest.mark.parametrize(
    "surface",
    [
        "tests/contracts/test_commit10_acceptance_retry_2_workflow.py",
...
def test_normal_buddy_remains_disabled_before_attempt_without_legacy_route() -> (
    ...
    assert (
        "workflow-delivery-v3-buddy-smoke-acceptance-retry-2.yml" not in normal
    )
    ...
    return FixedAcceptanceSuiteResult(
        ...
        scenarios=tuple(
            ...
            for index, (
                ...
                classification,
                diagnostics,
            ) in enumerate(scenarios, start=5)
...
def _execute_terminal_capture(  # noqa: PLR0913
    ...
    *,
    ...
    _finalize_retry_profile_for_capture_tests(monkeypatch)
    ...
    environment = {
        ...
        "INPUT_CONFIRM": "I_ACCEPT_DISPOSABLE_GITHUB_PACKAGES_PROBES_RETRY_2",
    }
...
def _finalize_retry_profile_for_capture_tests(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    profiles = cast(
        "tuple[Any, ...]",
        governance_module._GOVERNANCE_ACCEPTANCE_PROFILES,  # noqa: SLF001
    )
    monkeypatch.setattr(
        governance_module,
        "_GOVERNANCE_ACCEPTANCE_PROFILES",
        tuple(
            replace(profile, target_sha="a" * 40)
            if profile.package_coordinate.endswith("wdv3-acceptance.5")
            else profile
...
def test_confirmation_literal_is_exact_and_never_describes_workflow_as_inert() -> (
    ...
    document = _document()
    expected = "I_ACCEPT_DISPOSABLE_GITHUB_PACKAGES_PROBES_RETRY_2"
