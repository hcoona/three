...
@pytest.mark.parametrize(
    ("field", "value", "message"),
    [
        ("canonical_version", 123, "(?:canonical|NBGV.*version)"),
        ("canonical_version", "1", "(?:canonical|NBGV.*version)"),
        ("canonical_version", "1.2.3.4.5", "(?:canonical|NBGV.*version)"),
        ("canonical_version", "01.2.3", "(?:canonical|NBGV.*version)"),
        ("canonical_version", "1.two.3", "(?:canonical|NBGV.*version)"),
        ("canonical_version", " 1.2.3", "(?:canonical|NBGV.*version)"),
        ("sem_ver1", "", "(?:semVer1|sem_ver1)"),
        ("sem_ver2", 123, "(?:semVer2|sem_ver2)"),
        ("sem_ver1", " 1.2.3", "(?:semVer1|sem_ver1)"),
        (
            "npm_package_version",
            123,
        ...
        ("git_commit_id", 123, "gitCommitId"),
        ("git_commit_id", "e" * 39, "gitCommitId"),
        ("git_commit_id", "E" * 40, "gitCommitId"),
        ("git_commit_id", "g" * 40, "gitCommitId"),
        ("git_commit_id", "d" * 40, "exact target"),
        ("version_height", 0, "(?:versionHeight|version_height)"),
        ("version_height", True, "(?:versionHeight|version_height)"),
        ("version_height", "42", "(?:versionHeight|version_height)"),
        ("version_height", 42.5, "(?:versionHeight|version_height)"),
        ("public_release", "false", "(?:publicRelease|public_release)"),
        ("public_release", 0, "(?:publicRelease|public_release)"),
        (
            "node_api_result_digest",
            "a" * 64,
            "(?:node-api-result-digest|node_api_result_digest|digest)",
        ),
        (
            "node_api_result_digest",
            "sha256:" + ("a" * 63),
    ...
    ids=[
        ...
        "npm-invalid-prerelease-character",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("npm_package_version", "1.2.3+build.5"),
        ("npm_package_version", "1.2.3-beta.4+build.5"),
    ],
)
def test_nbgv_facts_accept_supported_version_forms(
    field: str, value: str
) -> None:
    """Accept canonical numeric versions and native npm SemVer forms."""
    facts = replace(_nbgv_facts(), **{field: value})

    validate_nbgv_facts(facts, target=TARGET)

    assert getattr(facts, field) == value
