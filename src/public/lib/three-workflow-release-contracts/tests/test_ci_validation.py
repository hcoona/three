...
EXPECTED_PHYSICAL_NAME_LENGTH = 84
...
def test_artifact_physical_name_format_length_and_determinism() -> None:
    """Map logical artifact refs to fixed digest-only physical names."""
    ...
    assert physical_name == f"{ARTIFACT_PHYSICAL_NAME_PREFIX}{expected_digest}"
    assert len(physical_name) == ARTIFACT_PHYSICAL_NAME_LENGTH
    assert ARTIFACT_PHYSICAL_NAME_LENGTH == EXPECTED_PHYSICAL_NAME_LENGTH
    assert re.fullmatch(r"three-ci-validation-[0-9a-f]{64}", physical_name)
...
def test_diagnostic_vocabularies_are_closed_and_exported() -> None:
    ...
    assert (
        "ci-validation-selector-assignments" in REGISTERED_CI_VALIDATION_KINDS
    )
    assert (
    ...
    assert (
        API_VERSIONS_BY_KIND["ci-validation-selector-assignments"]
