...
class DocumentationRegressionTests(unittest.TestCase):
    ...
    def test_runner_is_ephemeral_hash_locked_and_fail_closed(self) -> None:
        ...
        self.assertIn("-NoProfile -NonInteractive -EncodedCommand", runner)
        self.assertIn("ado token --output token", runner)
        self.assertIn("Remove-Item Env:PIP_INDEX_URL", runner)
        self.assertIn("--require-hashes --only-binary=:all: --no-deps", runner)
