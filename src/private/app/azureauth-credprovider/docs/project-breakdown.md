...
| CONFIG        | Configuration lead. Owns the configuration manager, change-plan semantics, persistent write policy, rollback, and removal behavior.   |
...
| 1.3   | ADAPTER-PY               | Python backend-helper evidence gate. Prototype backend discovery, fixed external helper invocation, helper ownership validation, and stronger integrity options such as file hash or signing.                                                                                      | Pass/fail decision is recorded. Release packaging cannot lock until helper ownership and integrity expectations are accepted.                                                                      |
...
| 2     | ARCH                     | Contract freeze. Define credential request, credential result, typed errors, canonical resource identity, cache-key schema, `ConfigurationChangePlan`, `DoctorCheck`, adapter-host result mapping, and `keyring-helper-v1` command, exit, stdout, stderr, and integrity contract.  | Versioned contract tests pass with fakes. Compatibility rules are explicit.                                                                                                                        |
| 3     | PLATFORM                 | Foundation platform. Create build and test skeletons, packaging skeleton, cross-platform process and filesystem abstractions, redaction, logging, correlation primitives, and Windows path-with-spaces tests.                                                                      | CI produces artifacts on the target OS matrix and runs contract and foundation tests.                                                                                                              |
| 4     | CONFIG                   | Configuration manager. Implement declarative change plans, ownership manifest, dry-run equivalence, conflict handling, scoped persistent writes, and removal metadata. Enforce no adapter direct writes.                                                                           | Golden tests cover Git, NuGet, Python, and npm plan application and removal. Yarn selectors are covered in read-only mode, with conditional write/remove extension points if the Yarn gate passes. |
| 5A    | ARCH and PLATFORM        | Minimal adapter-host scaffold. Implement protocol-mode bootstrap, stdout/stderr discipline, and fatal-error mapping against frozen contracts.                                                                                                                                      | Fake protocol adapters prove stdout discipline.                                                                                                                                                    |
...
| 5B    | PLATFORM                 | Installer and discovery scaffold. Define artifact-placement conventions and discovery probes using fake artifacts only. Do not package final ecosystem adapters in this phase.                                                                                                     | Fake adapters are discoverable on Windows, Linux, and macOS. Final packaging remains owned by ecosystem phases and Phase 15.                                                                       |
...
| 11    | ADAPTER-PY               | Python adapter. Implement Python keyring backend, `keyring-helper-v1` fixed external helper, keyring CLI shim, virtual environment, pipx, tox, and uv discovery, helper ownership and integrity checks, exact no-credential versus hard-failure mapping, and Python doctor checks. | Import-mode, subprocess-mode, helper contract, ownership/integrity, environment discovery, and error-mapping tests pass.                                                                           |
...
| 15    | QA and PLATFORM          | End-to-end hardening. Run cross-ecosystem tests, Windows-first acceptance, secret redaction audit, installer and uninstaller validation, file-locking tests, path-with-spaces tests, GUI Git validation, and headless CI validation.                                               | Release-candidate acceptance matrix passes.                                                                                                                                                        |
