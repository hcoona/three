...
This record freezes the version 1 shared contracts implemented in
...
plans, doctor checks, adapter-host result mapping, and `keyring-helper-v1`.
...
| ConfigurationChangePlan     |       1 | Declarative, owner-scoped changes only. Create, update, refresh, remove, atomic change-set, rollback state, manifest metadata, global scope, and CI temporary declaration-preservation semantics are explicit. Adapters do not apply writes directly. `WorkspaceReadOnly` plans carry no `ConfigurationChange` writes. Secret-bearing changes are flagged, require `containsCredentialMaterial=true`, and are redacted in `ToString()`. Value-writing npm `.npmrc` `_authToken` selectors and Yarn `.yarnrc.yml` `npmAuthToken`/`npmAuthIdent` selectors are intrinsically secret by target/key semantics: they must set `isSecretValue=true`, require `containsCredentialMaterial=true`, reject CR or LF regardless of the supplied secret flag, and are redacted even on invalid instances. Phase 2 Yarn bearer-token plans emit `npmAuthToken` and `npmAlwaysAuth`, not `npmAuthIdent`; `npmAuthIdent` remains covered for validation, redaction, and conflict detection when user/provided plans contain it, but value-writing Yarn `npmAuthIdent` changes are not accepted as product-owned write-plan output even when marked secret.                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
(`Npmrc` `_authToken` and `Yarnrc` `npmAuthToken`/`npmAuthIdent`) are treated as secret by
...
`npmAuthIdent`; existing or user-provided `npmAuthIdent` changes remain subject
to intrinsic-secret safety.
...
- a consistent credential-material flag. Any change with `isSecretValue=true`,
  and any value-writing `Npmrc` `_authToken` or `Yarnrc` `npmAuthToken`/`npmAuthIdent` change by
  selector semantics, makes `containsCredentialMaterial=true` mandatory, and
  inconsistent deserialized plans fail closed. Phase 2 bearer-token plans do not
  emit `npmAuthIdent`, but user/provided `npmAuthIdent` changes still must be
  marked `isSecretValue=true` and must not contain CR or LF;
...
- Yarn Berry CI-temporary plans must detect same-registry project-local
  ...
  `npmRegistries[registry].npmAuthToken` or
  `npmRegistries[registry].npmAuthIdent` entries, same-registry
  ...
  `npmAlwaysAuth: false`, because Yarn prefers scope auth over registry auth. A
  ...
  `npmAuthIdent` is validation/conflict-only in this contract: generated Yarn
  write plans use `npmAuthToken` plus `npmAlwaysAuth`, and value-writing Yarn
  `npmAuthIdent` changes are rejected even when the value is marked secret.

## keyring-helper-v1 Contract

The backend invokes the helper with an argv list, not through a shell:

```text
python-keyring get --protocol-version 1 --service <service> [--username <user>] --mode password|creds
```
...
- Responses must carry `contractMajor: 1`; missing versions are rejected.
...
required owner validation, required symlink rejection, and required SHA-256 digest
verification before helper execution. These integrity requirements are not
represented as opt-out booleans in the Phase 2 contract. Explicit `unspecified`
or unknown integrity policy enum values are invalid and must be rejected before
helper execution. JSON wire payloads must carry the owner-validation,
symlink-policy, and digest-policy fields explicitly; omitted integrity policy
metadata is rejected instead of defaulting to secure values.
