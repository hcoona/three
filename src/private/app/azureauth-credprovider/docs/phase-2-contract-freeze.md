...
The adapter command is fixed to `python-keyring`, while `keyring-helper-v2` is
the protocol contract ID rather than a separate argv token. Requests that
specify an unsupported `contractMajor`, another command, omit the fixed command,
specify `unspecified`/unknown modes, or provide a missing,
