...
Until AzureAuth issue #464 is implemented, GCM-compatible Dev Box WSL hosts
temporarily route `SilentOnly` requests through AzureAuth's WAM-first default
flow. The behavior may display UI, is reported as a distinct mechanism, and can
be forced or disabled by a WSL-only environment override.
