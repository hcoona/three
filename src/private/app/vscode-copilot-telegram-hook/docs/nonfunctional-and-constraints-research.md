...
- Purpose: ground non-functional conclusions and external constraints in the
  ...
  decisions from H-002 and H-003.
...
| NFR-002 | Once Telegram delivery credentials are provided, the solution shall handle their persistence and reuse through a secure user-appropriate secret-management approach rather than embedding plaintext secrets in reusable artifacts, and may treat secret-storage installation as an external prerequisite. | This is a security and operations requirement about how credentials are handled, not whether credentials are needed.        | H-001 plus product decision recorded in H-003.                                                                                   |
...
- Secure credential storage is a hard prerequisite; `gopass` / GNU Pass remain
  preferred mechanisms, but the product does not need to standardize or
  automate installation of the secret-storage system itself.
