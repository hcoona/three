...
internal static class AtlasCliApplication
{
    ...
    private static readonly byte[] SaveSnapshotSuccessBytes =
        "Save snapshot completed.\n"u8.ToArray();
    private static readonly byte[] SnapshotSurveySuccessBytes =
        ...
        if (TryParseRequestCommand(args, out RequestCommand command))
        {
            return command.Kind switch
            {
                ...
                RequestCommandKind.DefinitionIntake => await RunOperationAsync(
                        ...
                        cancellationToken)
                    .ConfigureAwait(false),
                RequestCommandKind.SaveSnapshot => await RunOperationAsync(
                        standardOutput,
                        standardError,
                        cancellation => operations.RunSaveSnapshotAsync(
                            command.RequestFilePath,
                            cancellation),
                        SaveSnapshotSuccessBytes,
                        includeDiscoveryFailureStage: false,
                        cancellationToken)
                    .ConfigureAwait(false),
                RequestCommandKind.SnapshotSurvey => await RunOperationAsync(
                        standardOutput,
                        standardError,
                        cancellation => operations.RunSnapshotSurveyAsync(
                            command.RequestFilePath,
                            cancellation),
                        SnapshotSurveySuccessBytes,
                        includeDiscoveryFailureStage: false,
                        cancellationToken)
                    .ConfigureAwait(false),
    ...
    private static bool TryGetRequestCommandKind(string command, out RequestCommandKind kind)
    {
        ...
        if (StringComparer.Ordinal.Equals(command, "save-snapshot"))
        {
            kind = RequestCommandKind.SaveSnapshot;
            return true;
        }

        if (StringComparer.Ordinal.Equals(command, "snapshot-survey"))
        {
            kind = RequestCommandKind.SnapshotSurvey;
            return true;
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (AtlasSafetyException exception)
        {
            ...
            exception is AtlasSaveReadException
