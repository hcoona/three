...
export class CopilotAcpClient {
  ...
  static async start(options: {
    ...
    const client = new CopilotAcpClient(processHandle);
  ...
  async initialize(): Promise<AcpInitializeResult> {
    const result = (await this.request('initialize', {
      ...
      clientCapabilities: {},
      clientInfo: {
        name: 'telegram-topic-session-bridge',
        title: 'Telegram Topic Session Bridge',
        version: '0.0.0',
      },
  ...
  private async handleIncomingMethod(message: {
    ...
    if (message.method === 'session/request_permission') {
      ...
      const params = message.params as AcpPermissionRequestParams | undefined;
      if (!params || !this.permissionHandler) {
      ...
      const result = await this.permissionHandler(params);
