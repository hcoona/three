...
That workspace file is now only a thin entry point that calls the shared scripts under `src/private/app/vscode-copilot-telegram-hook/scripts/`.
...
The user-level installer is:

```powershell
pwsh -NoLogo -NoProfile -File ./src/private/app/vscode-copilot-telegram-hook/Install-UserCopilotHook.ps1
```

For headless installation, pass `-TelegramBotToken` and `-TelegramChatId` (or set `TG_BOT_TOKEN` and `TG_CHAT_ID`) together with `-SkipSecretPrompt`.

The installer prompts for the Telegram bot token and chat ID when run interactively, validates their basic format, stores them in `gopass`, installs the PowerShell scripts into a stable user-owned directory, updates `~/.claude/settings.json` so VS Code loads the user-level hooks globally, and installs a user-level VS Code GitHub Copilot instruction file under `~/.copilot/instructions` so task summaries are generated consistently.
It supports `Auto`, `Copy`, `Hardlink`, and `Cow` install modes so the installed hook does not depend on the current repository checkout path or commit.
...
At runtime, the hook resolves the active workspace from the hook input payload instead of the script location.
That keeps `.copilot/notify-session.json`, `.copilot/notify-summary.json`, and `.copilot/notify-last-sent.json` scoped to the actual workspace even after a user-level installation.
...
The runtime still honors `TG_BOT_TOKEN` and `TG_CHAT_ID` from the process environment or the repo-root `.env` file as explicit overrides, but the default installation path uses `gopass`.
