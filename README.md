...
| Source subtree routing | `src/Directory.Build.props`             | `GitVersionBaseDirectory=$(RepoRoot)` for `src/**`; `IsPackable=true` for `src/public/lib/**` and `src/private/lib/**`; `IsPackable=false` for `src/public/app/**`, `src/private/app/**`, `src/lab/**`, and `src/sample/**`                                                                                                                                                                          | Source-subtree packability is routed centrally here, and the shared repo-root version base only needs narrower project-dir resets where local `version.json` files exist. |
| Private app subtree    | `src/private/app/Directory.Build.props` | `JsonSerializerIsReflectionEnabledByDefault=false`                                                                                                                                                                                                                                                                                                                                                   | Private apps now inherit the shared repo-root version base from `src/Directory.Build.props`, while the serializer setting remains private-app-specific.                   |
| Public app subtree     | `src/public/app/Directory.Build.props`  | `GitVersionBaseDirectory=$(MSBuildProjectDirectory)` reset for `src/public/app/**`                                                                                                                                                                                                                                                                                                                   | Public apps need project-dir semantics so app-local `version.json` files still win, while apps without a local file still fall back to the repo root.                     |
| Public library subtree | `src/public/lib/Directory.Build.props`  | `GitVersionBaseDirectory=$(MSBuildProjectDirectory)` reset for `src/public/lib/**`; `CentralPackageTransitivePinningEnabled=false`; `IsAotCompatible` helper; `StyleCop.Analyzers`; NuGet metadata; linked `stylecop.json`                                                                                                                                                                           | Public libraries need project-dir semantics so library-local `version.json` boundaries are preserved while still inheriting the rest of the shared library posture.       |
