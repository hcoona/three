...
After Groups N1/N2/N3, the live props graph is now override-oriented: the root `Directory.Build.props` owns the shared analyzer baseline, and every child `Directory.Build.props` only keeps subtree-local deltas that still need a narrower owner.

| Owner scope              | Live owner file                                             | Shared settings/packages still owned there                                                                                                                                                                                                                                                                                     | Why it remains there                                                                                                                               |
| ------------------------ | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| Repo-wide baseline       | `Directory.Build.props`                                     | `AnalysisLevel=latest`, `AnalysisMode=Recommended`, `TreatWarningsAsErrors=true`, `CodeAnalysisTreatWarningsAsErrors=true`, `EnforceCodeStyleInBuild=true`                                                                                                                                                                     | These settings now apply once for the whole C# repo and are no longer repeated in child props files.                                               |
| Shared public source set | `src/public/Directory.Build.props`                          | `GlobalPackageReference Include="Nerdbank.GitVersioning" Version="3.9.50"`                                                                                                                                                                                                                                                     | `src/public/app` and `src/public/lib` intentionally share the same NBGV owner, while `src/private/app` does not.                                   |
| Lab subtree              | `src/lab/Directory.Build.props`                             | `IsPackable=false`; `DotNet.ReproducibleBuilds`=`2.0.2`; `Microsoft.SourceLink.GitHub`=`8.0.0`                                                                                                                                                                                                                                 | These settings only apply to lab projects.                                                                                                         |
| Private app subtree      | `src/private/app/Directory.Build.props`                     | `RestorePackagesConfig=true`; `JsonSerializerIsReflectionEnabledByDefault=false`; `IsPackable=false`; `GitVersionBaseDirectory=$(MSBuildProjectDirectory)`; `DotNet.ReproducibleBuilds`=`2.0.2`; `Microsoft.CodeAnalysis.BannedApiAnalyzers`=`3.3.4`; `Microsoft.SourceLink.GitHub`=`8.0.0`; `Nerdbank.GitVersioning`=`3.9.50` | These settings are private-app-specific and intentionally do not flow to other source subtrees.                                                    |
| Public app subtree       | `src/public/app/Directory.Build.props`                      | `GitVersionBaseDirectory=$(MSBuildProjectDirectory)`; `DotNet.ReproducibleBuilds`=`1.2.25`; `Microsoft.CodeAnalysis.BannedApiAnalyzers`=`3.3.4`; `Microsoft.SourceLink.GitHub`=`8.0.0`                                                                                                                                         | These settings only apply to public apps; NBGV comes from the shared `src/public` owner above.                                                     |
| Public app nested child  | `src/public/app/ImageOcclusionEditor/Directory.Build.props` | `RestorePackagesConfig=true`                                                                                                                                                                                                                                                                                                   | This migrated public app still needs `packages.config` restore locally, without pushing that legacy requirement onto the whole public-app subtree. |
| Public library subtree   | `src/public/lib/Directory.Build.props`                      | `CentralPackageTransitivePinningEnabled=false`; `IsPackable=true`; `GitVersionBaseDirectory=$(MSBuildProjectDirectory)`; `IsAotCompatible` helper; `StyleCop.Analyzers`; NuGet metadata; linked `stylecop.json`                                                                                                                | These settings define the shared pack/library posture for `src/public/lib` only.                                                                   |
| Test baseline            | `tests/Directory.Build.props`                               | `IsPackable=false`                                                                                                                                                                                                                                                                                                             | Tests stay non-packable repo-wide, but otherwise inherit the root analyzer baseline directly.                                                      |
