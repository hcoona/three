...
After Groups N1/N2/N3, the live props graph is now override-oriented: the root `Directory.Build.props` owns the shared analyzer baseline, and the remaining child `Directory.Build.props` entries in the table only keep subtree-local deltas that still need a narrower owner.
...
| Private app subtree    | `src/private/app/Directory.Build.props` | `JsonSerializerIsReflectionEnabledByDefault=false`                                                                                                                                                                                                                                                                                                                                                   | This setting is private-app-specific and intentionally does not flow to other source subtrees.                                                  |
