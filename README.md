...
After Groups N1/N2/N3 plus the low-risk `GitVersionBaseDirectory` follow-up, the live props graph is now override-oriented: the root `Directory.Build.props` owns the shared analyzer baseline, while child `Directory.Build.props` files only keep subtree-local deltas and safe subtree-scoped version-base reuse that still need a narrower owner.
...
| Source subtree routing | `src/Directory.Build.props`             | `IsPackable=true` for `src/public/lib/**` and `src/private/lib/**`; `IsPackable=false` for `src/public/app/**`, `src/private/app/**`, `src/lab/**`, and `src/sample/**`                                                                                                                                                                                                                              | Source-subtree packability is routed centrally here so child props files only keep narrower subtree-local deltas.                                  |
| Lab subtree            | `src/lab/Directory.Build.props`         | `GitVersionBaseDirectory=$(RepoRoot)` for `src/lab/**`                                                                                                                                                                                                                                                                                                                                               | `src/lab` has multiple C# projects, inherits the root `version.json`, and has no nested local `version.json` boundaries to preserve.               |
| Private app subtree    | `src/private/app/Directory.Build.props` | `GitVersionBaseDirectory=$(RepoRoot)` for `src/private/app/**`; `JsonSerializerIsReflectionEnabledByDefault=false`                                                                                                                                                                                                                                                                                   | The subtree can safely share repo-root version computation, and the serializer setting remains private-app-specific.                               |
| Public library subtree | `src/public/lib/Directory.Build.props`  | `CentralPackageTransitivePinningEnabled=false`; `IsAotCompatible` helper; `StyleCop.Analyzers`; NuGet metadata; linked `stylecop.json`                                                                                                                                                                                                                                                               | These settings define the shared pack/library posture for `src/public/lib` only; packability itself now comes from `src/Directory.Build.props`.    |
| Test baseline          | `tests/Directory.Build.props`           | `GitVersionBaseDirectory=$(RepoRoot)` for `tests/**`; `IsPackable=false`                                                                                                                                                                                                                                                                                                                             | The entire `tests` subtree shares the root `version.json` today, so tests can reuse one repo-root version calculation without changing boundaries. |
