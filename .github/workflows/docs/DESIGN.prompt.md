...
Before any implementation-oriented design detail: this repository has **not** yet completed its per-project root migration. `AGENTS.md` says source code has not yet been moved into the final per-project subdirectories under `src/`. Your design must therefore treat that migration as a prerequisite for implementation and must not pretend the current repository layout already satisfies project-resolution requirements.
...
Do **not** add extra triggered top-level workflows for readiness checks, health monitors, governance, drift detection, or similar control-plane tasks. If a capability is still needed, keep it inside one of the three entry workflows or make it checked-in repository state.
