...
- RubyGems.org publicly documents reusable-workflow binding via GitHub's `job_workflow_ref` behavior and the separate Workflow Repository Owner/Name fields for cross-repository reusable workflows
...
- PyPI's documented GitHub Actions trusted-publishing setup anchors trust to the entry workflow filename and does not yet document reusable-workflow-path binding as a supported contract
...
- official build workflows with `require-provenance: true` emit verifier input material rather than final durable evidence: the build side produces `build-verification-input.json`, `attestation-manifest.json`, and the attestation bundle set under the provenance artifact, while `require-provenance` writes the final repository-controlled `artifact-evidence.json` to the protected `release-evidence` branch; same-path overwrites are disallowed and recovery uses the durable blob permalink rather than an expiring CI artifact URL
...
- burned, partially published, and fully withdrawn official identities, plus required periodic tag audits, are recorded in `.github/release-recovery-ledger.jsonl` using `recordType` values `{incident, audit}` under `CODEOWNERS` review, with a P0/P1 break-glass path for minimal emergency ledger updates followed by a reviewed cleanup PR; incident records now include `incidentId`, `selectedTargets`, strict key whitelists, and hold-window evidence fields for destructive stable-release recovery
- incident and audit ledger records both include `schemaVersion` and `recordType`; `disposition`, `publishedTargets`, `pendingTargets`, `withdrawnTargets`, `tagState`, `githubReleaseState`, `audit.scope`, and `audit.result` all use closed schemas or enums, `closedAt` is absent rather than null while an incident remains open, `fully-withdrawn` is a terminal incident disposition, and audit records require `closedAt` for both `followUpStatus = resolved` and `followUpStatus = not-required`; `automationId` and `scriptVersion` are an all-or-nothing pair for automated audit records
...
- maintenance branch retirement first drains queued, waiting, action-required, requested, or in-progress official runs from that branch with separate status-specific queries, performs active polling for up to 5 minutes with at least 30-second intervals before each re-check and at least five polls across the two propagation windows, verifies there is no still-open incident ledger entry for that release line, allows quarantine only for `open-before-publish` incidents, routes `open-partial-publish` incidents directly into the emergency-cleanup path, keeps immutable `openedAt` while moving `phaseStartedAt` forward on phase changes, permits quarantine to resume `active` when the blocking incident is resolved early, increments monotonic `drainAttempts` whenever newly created runs reappear after branch removal, escalates into the emergency-cleanup path once `drainAttempts == 3`, and uses that emergency path to keep the branch outside `production-<project-name>`, `production-tag-write-<project-name>`, and `production-evidence-write-<project-name>` while cancelling newly queued or waiting runs before restarting step 0; normal retirement then removes the exact branch from all three protected environments plus `.github/official-caller-refs.json` and `.github/publish-trust-inventory.json`, then transitions the same planned-change window to `cooldown`, with rollback restoring the checked-in caller-ref and inventory entries before the deployment-policy entries
...
- official recovery monitoring uses five control-plane monitors: a 6-hour approval-age monitor with its own external heartbeat, an event-driven post-tag failure monitor for non-success official runs after `create-release-tag` settled as either `created` or `no-op`, a scheduled post-tag-monitor health check with its own external heartbeat, a daily open-incident freshness monitor that enforces the 14-day escalation, `eligibleDeleteAt` reminders, and the 24-hour discrepancy-follow-up SLA, and a 7-day operational audit with its own external heartbeat
...
- any open incident with `disposition` in `{open-before-publish, open-partial-publish}` is re-checked daily against live state and escalates automatically to release-engineering owners with current live-state evidence once it reaches 14 days; unresolved `discrepancy-found` audit follow-ups also alert after 24 hours
...
- `.github/planned-change-windows.json` is a strict schema with `schemaVersion: 1` and a `windows` array; each window has `windowId`, `operation`, `status`, `projectName`, `releaseLine`, immutable `openedAt`, `phaseStartedAt`, `expiresAt`, `openedBy`, plus optional `linkedIncidentId`, `closedAt`, `cooldownUntil`, and `notes`; onboarding `active` windows are capped at 24 hours, retirement `active` windows at 8 hours, `quarantine` windows at 7 days with `phaseStartedAt`/`expiresAt` rewritten for the quarantine phase while `openedAt` stays unchanged, and `cooldown` records enforce a minimum 48-hour gap before the same tuple can open a new active window
...
- `open-before-publish` requires `publishedTargets = []` and `pendingTargets = selectedTargets`, `open-partial-publish` requires both `publishedTargets` and `pendingTargets` to be non-empty, `recovered` requires `pendingTargets = []`, `abandoned-after-partial-publish` requires both sets to be non-empty, and `fully-withdrawn` requires `pendingTargets = []` plus `withdrawnTargets = publishedTargets`
