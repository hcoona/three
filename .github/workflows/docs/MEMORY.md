...
- timeout defaults are explicit: `preflight-check`/resolve/static-analysis `15`, Ubuntu builds `30`, Windows builds `45` because hosted Windows runners have higher startup and restore/test overhead, `require-provenance` `30`, `confirm-publish-state` `25`, publish jobs `15`, tag-management jobs `10`, and `ci-passed`/`release-complete` `15`
...
- official build workflows with `require-provenance: true` emit verifier input material rather than final durable evidence: the build side produces deterministic `build-verification-input.json` inside the main build artifact, while `require-provenance` generates the final `attestation-manifest.json` plus verified attestation bundle set, writes the repository-controlled `artifact-evidence.json` to the protected `release-evidence` branch, and records recovery against the durable blob permalink rather than an expiring CI artifact URL; same-path overwrites are disallowed
...
- burned, partially published, delisted, and fully withdrawn official identities, plus required periodic tag audits, are recorded in `.github/release-recovery-ledger.jsonl` using `recordType` values `{incident, audit}` under `CODEOWNERS` review, with a P0/P1 break-glass path for minimal emergency ledger updates followed by a reviewed cleanup PR; incident records now include `incidentId`, monotonic `revision`, required `releaseLine`, `selectedTargets`, strict key whitelists, and hold-window evidence fields for destructive stable-release recovery
- incident and audit ledger records both include `schemaVersion` and `recordType`; `disposition`, `publishedTargets`, `pendingTargets`, `deprecatedTargets`, `delistedTargets`, `removedTargets`, `retainedTargets`, `tagState`, `githubReleaseState`, `audit.scope`, and `audit.result` all use closed schemas or enums, `closedAt` is absent rather than null while an incident remains open, terminal published incident states require exhaustive target accounting across `retainedTargets ∪ deprecatedTargets ∪ delistedTargets ∪ removedTargets`, `fully-withdrawn` is a terminal incident disposition, and audit records require `closedAt` for both `followUpStatus = resolved` and `followUpStatus = not-required`; `automationId` and `scriptVersion` are an all-or-nothing pair for automated audit records
...
- official recovery monitoring now uses seven control-plane monitors: a 2-hour drift monitor with its own external heartbeat, a 6-hour approval-age monitor with threshold-crossing deduplication and its own external heartbeat, an event-driven post-tag failure monitor, a scheduled post-tag-monitor health check with its own external heartbeat, a daily open-incident freshness monitor with a 7/14/21/30-day escalation ladder, a 7-day operational audit with its own external heartbeat, and a daily governance-and-runbook freshness monitor with its own external heartbeat
...
- `open-before-publish` requires `publishedTargets = []` and `pendingTargets = selectedTargets`, `open-partial-publish` requires both `publishedTargets` and `pendingTargets` to be non-empty, `recovered` requires `pendingTargets = []` plus `retainedTargets = publishedTargets`, `abandoned-after-partial-publish` requires both sets to be non-empty, and `fully-withdrawn` requires `pendingTargets = []` plus `deprecatedTargets ∪ delistedTargets ∪ removedTargets = publishedTargets`
