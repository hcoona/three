...
- `official.yml` includes a `preflight-check` job inside the repository-scoped `control-plane-monitoring` environment, and that job derives and validates the required target-specific publish environments plus the dedicated tag-write and evidence-write environments before any production approval is consumed; each protected environment also carries a required `CONTROL_PLANE_ENVIRONMENT_ROLE` variable so downstream jobs fail closed if GitHub auto-creates a missing environment without protection
...
- `preflight-check` also validates that `control-plane-monitoring` exposes the exact repository-wide protected caller-ref set frozen from `main`, so any onboarding or retirement mismatch on any supported official branch holds every `official.yml` run rather than only the selected project's release path
...
- `control-plane-monitoring` has its own independent deployment branch policy, that policy must equal the exact repository-wide protected control-plane branch set from frozen `main`, and it must be updated and drift-checked alongside the affected publish/tag/evidence environments whenever that set changes; no environment in this design may use deployment tag policies
...
- CI includes an explicit `trusted-release-inventory` job that uses trusted validator logic from `main` to validate the PR merge commit's candidate post-change control-plane data, including `entryWorkflowPath`, the project-scoped `allowedCallerRefs` subset derived from the candidate `.github/official-caller-refs.json`, and the per-target `publishExecutionPath`, `environment`, `authMechanism`, optional `trustedPublisherSelector`, optional `documentedOidcAudience`, and optional `oidcAudienceEndpoint` fields against the candidate `.github/publish-trust-inventory.json`; CI fails on any mismatch whether or not the inventory file itself changed
...
- maintenance branch onboarding is a bounded control-plane transition and repository-wide official-release hold in this design: create and protect the branch first, update GitHub-side deployment-branch policy and any required registry-side selector or secret change next, then land the reviewed `main` control-plane change, and lift the hold only after GitHub-side and checked-in state converge; the drift monitor opens `high-nonpage` immediately and pages only after prolonged mismatch or overlap with any active official run
- maintenance branch retirement is also a bounded control-plane transition and repository-wide official-release hold: first drain or settle queued and in-progress official runs for that branch, then land the reviewed `main` removal of that caller ref from `.github/official-caller-refs.json` and from the matching project's `allowedCallerRefs` in `.github/publish-trust-inventory.json` while preserving the rest of the project entry, then remove the branch from the protected environments, and page only if the resulting mismatch persists too long or overlaps any active official run; if retirement cannot finish in one operator session, the supported alternatives are to finish it immediately or restore the pre-retirement state
