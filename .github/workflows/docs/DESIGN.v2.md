...
The externally exposed release and validation entry workflows are:

- `ci.yml`
- `buddy.yml`
- `official.yml`

No additional triggered top-level workflows are release entry points in this design. A scheduled or manually dispatched dependency-maintenance workflow such as `renovate.yml` is allowed only when it has no release authority, uses explicit least-privilege job permissions from a workflow-level `permissions: {}` baseline, cannot call release mutation workers, and does not mint publish credentials or protected-ref bypass credentials. It may use a dedicated dependency-maintenance token to create dependency branches and pull requests, but automerge must remain disabled unless a later reviewed token contract explicitly proves that protected branches and release refs cannot be mutated or bypassed.
...
- The only externally exposed release and validation workflows are `ci.yml`, `buddy.yml`, and `official.yml`; scheduled dependency-maintenance workflows are allowed only without release authority.
