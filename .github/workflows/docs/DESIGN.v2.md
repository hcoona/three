...
4. **`trusted-release-inventory`**:
    ...
    - Checks out the PR merge commit with `persist-credentials: false` and runs the repository-side drift check for `.github/publish-trust-inventory.json`. This job must recompute the post-change trust-bearing state from the checked-in control-plane files, using `.github/official-caller-refs.json` as the authoritative repository-side source for the normalized fully qualified `allowedCallerRefs` set, and compare the exact normalized values of `entryWorkflowPath`, `allowedCallerRefs`, `publishWorkflowPaths`, `targetEnvironments`, `targetAuthMechanisms`, `targetTrustedPublisherSelectors`, and `targetOidcAudiences`. Any missing inventory update, stale mapping, malformed schema, mismatched environment mapping, mismatched auth mechanism, mismatched trusted-publisher selector, or mismatched OIDC audience mapping is a hard failure.
    - The same job must also validate `.github/planned-change-windows.json` against the Section 4 schema whenever that file changes, including the strict key whitelists, unique `windowId` enforcement, operation/status closed sets, RFC 3339 UTC timestamps, per-status maximum window duration, and `linkedLedgerEntry` requirements for quarantine windows.
    - The same job must also validate `.github/release-recovery-ledger.jsonl` against the Section 7 schema whenever that file changes, including the strict key whitelists, closed sets, conditional `runAttempt`, `workflowRunUrl`, `evidenceUrl`, `closedAt`, `automationId`, and `scriptVersion` rules, canonical target ordering, the `selectedTargets` / `publishedTargets` / `pendingTargets` partition invariants, the `withdrawnTargets` subset invariants, the disposition-specific non-empty-set rules for `open-partial-publish`, `recovered`, and `abandoned-after-partial-publish`, and the presence-and-absence rules for the hold-window evidence fields used in destructive stable-release recovery.
...
- **Trusted-publisher selector matrix:** `npmjs` binds the repository, the calling workflow filename `.github/workflows/official.yml`, and the project-scoped production environment; it does not use the called reusable publish workflow as the trust anchor. `PyPI` binds the repository, the called reusable publish workflow `.github/workflows/_publish-pypi.yml` when the publish happens through that reusable workflow, the project-scoped production environment, and the documented OIDC audience `pypi`. `RubyGems.org` binds the repository, the called reusable publish workflow `.github/workflows/_publish-rubygems.yml` when the publish happens through that reusable workflow, and the project-scoped production environment. Exact branch or tag binding is not assumed documented in any provider UI and therefore is not part of this design.
...
No top-level keys other than `schemaVersion` and `windows` are allowed. `windows` must be an array of unique objects keyed by `windowId`. Every window object must contain exactly `windowId`, `operation`, `status`, `projectName`, `releaseLine`, `openedAt`, `expiresAt`, and `openedBy`, plus optional `linkedLedgerEntry` and `notes`. `operation` must use the closed set `{onboarding, retirement}`. `status` must use the closed set `{active, quarantine}`. `openedAt` and `expiresAt` must be RFC 3339 UTC timestamps. `projectName` and `releaseLine` must each match the same validation rules used elsewhere in this design. When `status = active`, `expiresAt - openedAt` must be no more than 4 hours and `linkedLedgerEntry` must be absent. When `status = quarantine`, `operation` must be `retirement`, `expiresAt - openedAt` must be no more than 7 days, and `linkedLedgerEntry` is required and must identify the exact open ledger record that justifies the quarantine. At most one non-expired window may exist for the same `{operation, projectName, releaseLine}` tuple.
...
The inventory uses fully qualified Git refs under `allowedCallerRefs`, repository-relative workflow paths, a target-to-environment mapping, an explicit target-to-auth mapping, a target-to-selector mapping for trusted-publisher-backed targets, and a target-to-documented-audience mapping for the subset of targets whose provider documents an audience. `allowedCallerRefs` is not an independent source of truth: it must exactly mirror the normalized `refs` array from `.github/official-caller-refs.json`.

No top-level keys other than `schemaVersion`, `entryWorkflowPath`, `allowedCallerRefs`, `publishWorkflowPaths`, `targetEnvironments`, `targetAuthMechanisms`, `targetTrustedPublisherSelectors`, and `targetOidcAudiences` are allowed. The validator must enforce this as a strict top-level key whitelist equivalent to JSON Schema with `additionalProperties: false`. `publishWorkflowPaths`, `targetEnvironments`, and `targetAuthMechanisms` must contain exactly the five official target keys. `targetTrustedPublisherSelectors` must contain exactly the currently trusted-publisher-backed targets `{npm:official, pypi:official, rubygems:official}` and no others. Every selector object must contain exactly `selectorModel`, `workflowPath`, and `environment`; `selectorModel` must use the closed set `{entry-workflow, reusable-workflow}`. `targetOidcAudiences` must contain exactly the targets whose provider documents a required audience in this design; as of v2.5 that set is `{pypi:official}`.
...
8. **`confirm-publish-state`**:
    ...
    - `permissions: { contents: read }`
    - Re-queries every selected official target against its live remote system and emits `confirmed-published-targets` as a compact ordered JSON array in canonical official-target order, serialized exactly as compact JSON equivalent to `jq -c`. This job is the authoritative workflow-side source of truth for `publishedTargets`; machine-readable publish outputs and step summaries are diagnostic hints only.
...
```json
{
    ...
    "verifiedRepositoryOwnerId": "12345678",
    "verifiedEnvironment": "production-example-project"
}
```

No top-level keys other than those shown above are allowed. `schemaVersion` must equal `1`. `artifactManifestSha256` must match exactly `[0-9a-f]{64}`. `verifiedRepository` must be the canonical `owner/repo` string for the run. `verifiedRef`, `verifiedSourceSha`, `verifiedJobWorkflowRef`, `verifiedWorkflowSha`, and `verifiedRepositoryOwnerId` must match the attestation verification result exactly. `verifiedEnvironment` is optional and must be absent rather than `null` when the attested build job did not execute inside a GitHub environment. The later `require-provenance` job must copy that evidence record to the protected durable evidence branch described in Section 8 and must emit the immutable `artifactManifestEvidenceUrl`; an expiring GitHub Actions artifact URL is never a valid durable evidence reference.
...
**Artifact retention:** CI artifacts are an ephemeral hand-off mechanism, not permanent release storage. Recommended defaults: `retention-days: 7` for PR and buddy runs, `retention-days: 90` for official runs. The longer official retention window is intentional recovery budget for partial publishes, approval delays, and post-incident evidence comparison, and it deliberately exceeds GitHub's 30-day deployment-approval expiry by a full additional 60 days. It does not eliminate the dead-end case where artifacts expire and the protected branch has since moved, so Section 7 still defines that as a separate recovery boundary.
...
14. If official publish jobs fail with authentication or authorization errors immediately after a new maintenance branch, trusted workflow path, protected control-plane branch change, or explicit registry-secret rotation was introduced, diagnose the mismatch direction explicitly. If repository-side publish trust inventory preflight fails first, fix `.github/publish-trust-inventory.json` or roll back the external auth configuration change before retrying. If publish trust inventory preflight succeeds but publish still fails at the registry, verify and restore the external auth configuration for the expected selector or secret model: for npmjs, PyPI, and RubyGems.org verify the trusted-publisher selector configuration for the checked-in workflow path, environment, and documented audience; for NuGet.org verify the environment secret name, scope, and active API key; for npm also verify the deterministic dist-tag choice. Treat both cases as control-plane configuration drift, not as a package-content defect. The repository must maintain a registry-specific rollback runbook for NuGet.org, npmjs, PyPI, and RubyGems.org that records the exact UI path, API call, or support-escalation path used to remove, rotate, or restore the relevant trusted-publisher selector or explicit credential for each registry.
