...
| Generate or verify attestations in `require-provenance` | `id-token: write`, `attestations: write` |
...
Because GitHub's documented attestation generation examples require both `id-token: write` and `attestations: write`, this design confines those scopes to `require-provenance`. Official build jobs must request only `contents: read`, build the artifact set, and upload deterministic build outputs plus build-side metadata. `require-provenance` then downloads that already-built artifact set, generates and verifies attestations in a separate trusted gate job, and writes the durable evidence record. Official build jobs must never receive `id-token: write` or `attestations: write` in this design.
...
**Job timeouts:** Every job must declare `timeout-minutes`, and workflow linting enforced through `hk`/`actionlint` should fail if any job omits it. Recommended defaults: `preflight-check`, resolution jobs, and static-analysis jobs `15`; Ubuntu build jobs `30`; Windows C# build jobs `45` because hosted Windows runners have materially higher startup and .NET restore/build/test overhead than Ubuntu runners; `require-provenance` `30` because it now generates attestations after downloading the full build artifact, verifies the attestation set, serializes evidence-branch writes, and performs immutable read-back verification on the evidence branch; publish jobs `15`; `confirm-publish-state` `25` because it performs bounded remote rechecks in parallel across the selected destinations and still needs time for runner startup, checkout, locked-tool bootstrap, and output serialization; lightweight tag-management jobs `10`; and terminal gate jobs `ci-passed` and `release-complete` `15` because those jobs must perform a read-only checkout and bootstrap `jq` from the locked `mise` toolchain before running their jq assertions. Some YAML snippets below omit `timeout-minutes` only for brevity; concrete workflow files must still declare it.
...
- **Alert routing contract:** Every page-worthy control-plane monitor in this design must route alerts to the release-engineering primary on-call destination plus a durable team-visible channel, and every non-page discrepancy alert must either open or update a tracked follow-up issue in the same operation. Alert destinations, acknowledgment SLA, and escalation ladder are part of the reviewed control-plane contract rather than an implementation detail.
- **Live control-plane drift detection:** In addition to near-real-time administration alerts, the repository must run a dedicated scheduled control-plane drift monitor at least every 2 hours. This monitor is a named control-plane monitor with its own external dead-man's-switch heartbeat and must query every live `production-<project-name>`, `production-tag-write-<project-name>`, and `production-evidence-write-<project-name>` deployment branch policy, compare the exact short-branch set to `.github/official-caller-refs.json`, and verify that `.github/official-caller-refs.json` plus `.github/publish-trust-inventory.json` are byte-identical across every branch currently in the protected control-plane branch set. Outside a non-expired matching `active` or `quarantine` record in `.github/planned-change-windows.json`, any mismatch between live GitHub state and the checked-in caller-ref registry is a control-plane incident even if no release is currently running. `cooldown` records document recent changes but do not authorize live drift. Drift automation must also fail if `.github/planned-change-windows.json` contains an expired window that has not been removed.
...
- **Trusted-publisher selector matrix:** GitHub's documented OIDC claims include `workflow_ref` and, for reusable workflows, `job_workflow_ref`; GitHub does not document a `caller_workflow_ref` claim. This design therefore models provider trust using provider-documented configuration contracts rather than inventing a cross-provider undocumented claim abstraction. `npmjs` publicly documents trusted publishing with repository owner, repository name, workflow filename, optional environment, and documented OIDC audience `npm:registry.npmjs.org`; npm's public docs are silent on reusable-workflow selectors, so this design models `npm:official` through the documented entry workflow `.github/workflows/official.yml` plus the project-scoped production environment. `PyPI` publicly documents trusted publishing with repository identity, workflow filename, and documented OIDC audience `pypi`; PyPI's public setup docs are silent on reusable-workflow UI configuration, so this design also models `pypi:official` through the documented entry workflow `.github/workflows/official.yml` plus the project-scoped production environment. `RubyGems.org` publicly documents reusable-workflow support with separate workflow-repository owner/name fields, so this design models `rubygems:official` through the called reusable publish workflow `.github/workflows/_publish-rubygems.yml`, the caller repository identity, the workflow repository identity, and the project-scoped production environment. Exact branch, tag, or commit-SHA binding is not assumed documented in any provider UI and therefore is not part of this design.
...
```json
{
    ...
    "targetTrustedPublisherSelectors": {
        ...
        "rubygems:official": {
            "providerContract": "reusable-workflow-ui",
            "repositoryOwner": "hcoona",
            "repositoryName": "three",
            "workflowPath": ".github/workflows/_publish-rubygems.yml",
            "workflowRepositoryOwner": "hcoona",
            "workflowRepositoryName": "three",
            "environment": "production-example-project"
        }
    },
    "targetOidcAudiences": {
...
No top-level keys other than `schemaVersion`, `entryWorkflowPath`, `allowedCallerRefs`, `publishWorkflowPaths`, `targetEnvironments`, `targetAuthMechanisms`, `targetTrustedPublisherSelectors`, and `targetOidcAudiences` are allowed. The validator must enforce this as a strict top-level key whitelist equivalent to JSON Schema with `additionalProperties: false`. `publishWorkflowPaths`, `targetEnvironments`, and `targetAuthMechanisms` must contain exactly the five official target keys. `targetTrustedPublisherSelectors` must contain exactly the currently trusted-publisher-backed targets `{npm:official, pypi:official, rubygems:official}` and no others. Every selector object must contain exactly `providerContract`, `repositoryOwner`, `repositoryName`, `workflowPath`, and `environment`, plus optional `workflowRepositoryOwner` and `workflowRepositoryName`. `providerContract` must use the closed set `{entry-workflow-ui, reusable-workflow-ui}`. `workflowRepositoryOwner` and `workflowRepositoryName` must be absent for `entry-workflow-ui` selectors and required for `reusable-workflow-ui` selectors. `targetOidcAudiences` must contain exactly the targets whose provider publicly documents a required audience in this design; as of v2.6 that set is `{npm:official, pypi:official}`.
...
1. **`preflight-check`**:
    ...
    - This job must not perform repository checkout. It must mint two dedicated GitHub App installation tokens just in time: an environment-reader token for GitHub Environments and deployment branch policy reads, and a separate ruleset-auditor token for authoritative Repository Rulesets reads including `bypass_actors`. A long-lived PAT is not the normal path. The GitHub App private keys must be stored as organization-level Actions secrets scoped only to this repository, not as repository-level secrets. If a temporary fallback secret is ever required before the GitHub App path exists, it must be treated as an emergency-only exception, kept out of repository-level secrets, and removed once the GitHub App path is in place. Treat the default `GITHUB_TOKEN` as insufficient for this job; weakening or skipping the verification is unsupported. Every minted installation token must be masked before first use.
    ...
    - Before any approval is consumed, this job must also verify through the refs API that `refs/heads/release-evidence` already exists. Ruleset presence alone is insufficient because rulesets do not create the branch.
...
4. **`clean-build`** (`build-csharp` / `build-python` / `build-jsts` / `build-ruby`):
    ...
    - Uses the same four static conditional build jobs pattern as `buddy.yml`, with `permissions: { contents: read }`, `secrets: {}`, and the required `with:` inputs wired from `build-scope: release`, `needs.resolve-context.outputs.project-path`, `needs.resolve-context.outputs.project-name`, `checkout-ref: ${{ github.sha }}`, and `require-provenance: true`. Each build job depends on both `resolve-context` and `static-analysis`. Only the language-matching build job executes; the others are skipped. These build jobs must never mint OIDC or attestation scopes; those scopes belong exclusively to `require-provenance`.
...
5. **`require-provenance`**:
    ...
    - Uses job-level concurrency `group: official::release-evidence` with `cancel-in-progress: false` so evidence-branch writes are serialized across projects. It downloads the single language-matching main build artifact, validates `build-verification-input.json`, recomputes the SHA-256 of the downloaded `artifact-manifest.json`, generates the attestation set for every manifest-selected publication file, constructs `attestation-manifest.json`, verifies the deterministic `attestations/` layout, and verifies binding to the exact workflow run, repository identity, source commit, project identity, version, and build artifact manifest for this release attempt. The validation is exact: every manifest entry selected for publication must have exactly one attestation-manifest entry and exactly one matching bundle file, and no extra attestation-manifest entry or extra bundle file is allowed.
    ...
    - `require-provenance` is the sole producer of the final durable `artifact-evidence.json`, `attestation-manifest.json`, and verified `attestations/` bundle set. The build job must not pre-populate any `verified*` field. After validation, this job must write an immutable evidence directory to the protected durable evidence branch at `.github/release-evidence/<project-name>/<version>/<source-commit>/runs/<github.run_id>-attempt-<github.run_attempt>/` containing exactly `artifact-evidence.json`, `artifact-manifest.json`, `build-verification-input.json`, `attestation-manifest.json`, and the verified `attestations/` bundle files generated in this job. Same-path overwrites are unsupported. The read-back verification must use the committed blob's content-addressed Git blob permalinks by blob SHA, not mutable branch/path URLs, and must verify byte equality for `artifact-evidence.json`, `artifact-manifest.json`, `build-verification-input.json`, and `attestation-manifest.json` before succeeding. This write path is mandatory, not best-effort. `artifactEvidenceUrl` for any later recovery ledger entry must point to the immutable blob permalink of the committed `artifact-evidence.json`. A missing durable evidence branch, evidence-writer credential failure, write failure, attempted overwrite, or byte-mismatch on the read-back verification is a hard failure.
    - Emits `artifact-evidence-url` as a workflow output so later recovery and audit steps can reference the durable evidence record directly.
...
| `require-provenance` | `boolean` | No | When `true`, the build workflow must emit the deterministic build-side metadata consumed later by `require-provenance`; the build workflow itself still must not mint attestations; defaults to `false` and is valid only when `build-scope = release` |
...
When `require-provenance = true`, the build workflow must place `build-verification-input.json` in the main build artifact next to `artifact-manifest.json`. The build workflow must not mint attestations, must not upload Sigstore bundles, and must not precompute `attestation-manifest.json`; those steps happen only later inside `require-provenance`. `artifact-evidence.json` is not part of the build-produced artifact; it is produced only later by `require-provenance` after attestation generation and verification succeeds. `build-verification-input.json` is internal control-plane metadata rather than a GitHub Release asset and uses a fixed exact schema:
...
No top-level keys other than those shown above are allowed. `schemaVersion` must equal `1`. `workflowRunAttempt` must equal the positive integer value of `github.run_attempt` for that build-producing run. `artifactManifestSha256` must match exactly `[0-9a-f]{64}`. This file records only build-side facts and must never contain any `verified*` claim.

After the build artifact is downloaded, `require-provenance` must generate `attestation-manifest.json` itself using this exact schema:
...
```json
{
    ...
    "artifactName": "build-output-example-project",
    "artifactManifestSha256": "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef",
    "artifactManifestBlobUrl": "https://github.com/hcoona/three/blob/<blob-sha>/.github/release-evidence/example-project/1.2.3/<source-commit>/runs/1234567890-attempt-1/artifact-manifest.json",
    "buildVerificationInputSha256": "76543210fedcba9876543210fedcba9876543210fedcba9876543210fedcba98",
    "buildVerificationInputBlobUrl": "https://github.com/hcoona/three/blob/<blob-sha>/.github/release-evidence/example-project/1.2.3/<source-commit>/runs/1234567890-attempt-1/build-verification-input.json",
    "attestationManifestSha256": "89abcdef0123456789abcdef0123456789abcdef0123456789abcdef01234567",
    "attestationManifestBlobUrl": "https://github.com/hcoona/three/blob/<blob-sha>/.github/release-evidence/example-project/1.2.3/<source-commit>/runs/1234567890-attempt-1/attestation-manifest.json",
    ...
    "verifiedBundles": [
        {
            ...
            "bundleBlobUrl": "https://github.com/hcoona/three/blob/<blob-sha>/.github/release-evidence/example-project/1.2.3/<source-commit>/runs/1234567890-attempt-1/attestations/artifact-file-name.ext.sigstore.json",
...
`_publish-npm.yml` must treat tarball idempotency and dist-tag idempotency as separate checks. When the package version already exists and the remote tarball identity matches the local artifact set, the workflow must still query the current owner of every requested tag in `dist-tags`, and must also enumerate the full remote dist-tag set that already points at that exact version. If a requested tag already points to the same version, preserve it and continue. If a requested tag is absent, attach it to the same version. If a requested tag points to a different version, the workflow may move that tag forward only when the current run's version has higher SemVer precedence within that same deterministic tag family; it must never move `latest`, a prerelease channel tag, or a maintenance-line tag backward or sideways to an older or unrelated version. Any unexpected pre-existing reserved-family tag already pointing to that version but absent from the requested deterministic array is a hard failure, not a tolerated leftover. `_publish-npm.yml` must emit the exact validated ordered JSON array through the `applied-dist-tags` output, and official `release-complete` must compare that output to the deterministic tag array derived in `resolve-context`.
