...
4. **`clean-build`** (`build-csharp` / `build-python` / `build-jsts` / `build-ruby`):
    ...
    - Uses the same four static conditional build jobs pattern as `buddy.yml`, with `permissions: { contents: read, id-token: write, attestations: write }`, `secrets: {}`, and the required `with:` inputs wired from `needs.resolve-context.outputs.project-path`, `needs.resolve-context.outputs.project-name`, `checkout-ref: ${{ github.sha }}`, and `require-provenance: true`. Each build job depends on both `resolve-context` and `static-analysis`. Only the language-matching build job executes; the others are skipped.
