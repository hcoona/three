...
2. **`resolve-context`**:
    ...
    - **Outputs:** `tag-name`, `language`, `project-name`, `project-path`, `version`, `release-line`, `targets` (JSON array of filtered official targets), `npm-dist-tags` (JSON array when `npm:official` is selected).
...
6. **`create-release-tag`**:
    ...
    - The job name and approval context shown to reviewers must include the project name, resolved version, caller ref, and short source commit so concurrent approval requests for the same project are distinguishable.
...
7. **Publish jobs** (static conditional, one job per official ecosystem-destination pair):
    ...
    - Every environment-gated publish job name and first summary line must include the target name, project name, resolved version, caller ref, and short source commit so concurrent approval requests for the same project are distinguishable in the approval UI and in notifications.
...
8. **`confirm-publish-state`**:
    ...
    - `permissions: { contents: read }`
    - Re-queries every selected official target against its live remote system and emits `confirmed-published-targets` as an ordered JSON array in canonical official-target order. This job is the authoritative workflow-side source of truth for `publishedTargets`; machine-readable publish outputs and step summaries are diagnostic hints only.
    - Remote confirmation must use bounded retry with backoff to absorb registry eventual consistency. A single immediate `not found` response after a publish attempt is `unknown`, not authoritative absence. The job must retry each selected target at least three times with increasing delays before concluding that a version, dist-tag, release asset, or GitHub Release state is missing.
    - For `npm:official`, this job must also verify the full ordered `dist-tags` array emitted by `resolve-context`, including any `latest` promotion for the highest supported stable maintenance line.
...
| `_publish-npm.yml`      | `caller-ref`   | `string`  | Yes      | Exact fully qualified caller ref such as `refs/heads/main` or `refs/heads/release/example-project/v1.2.x` |
| `_publish-npm.yml`      | `release-line` | `string`  | Yes      | Resolved release line such as `1.2.x` used to validate maintenance-line tag semantics |
| `_publish-npm.yml`      | `dist-tags`    | `string`  | Yes      | Ordered JSON array of explicit npm dist-tags to write (`["buddy"]`, `["latest"]`, `["release-v1.2"]`, or `["release-v1.2","latest"]`) |
...
| -------------------- | ------------------ | -------- | ----------- |
| `_publish-npm.yml`   | `applied-dist-tags` | `string` | Ordered JSON array of the exact npm dist-tags that the workflow validated and applied or confirmed as already present |
