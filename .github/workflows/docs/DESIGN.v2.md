...
The split axis is still **ecosystem (tooling)**, not destination, but official trusted publishing is now one explicit exception. GitHub OIDC binds external trust to the workflow identity that actually mints the token, and the public provider contracts for npmjs and PyPI remain entry-workflow-centric while RubyGems.org documents a different reusable-workflow contract. To remove that cross-provider ambiguity entirely, official trusted-publisher-backed jobs (`npm:official`, `pypi:official`, and `rubygems:official`) run directly in `official.yml`. Shared non-auth logic for those jobs may still live in reviewed composite actions or helper scripts, but the OIDC-minting publish job itself stays in the entry workflow. Reusable publish workflows remain for buddy GPR publishes plus the non-trusted-publisher official surfaces (`nuget:official` and `github:official`). Each publish call or direct job still mutates **exactly one** destination.
...
**Protected environments:** Official releases use three protected GitHub environments per releasable project plus one repository-scoped audit environment. `production-<project-name>` is the approval gate and OIDC environment claim for official publish jobs. `production-tag-write-<project-name>` is a separate tag-reservation environment used only by `create-release-tag`; it carries the release-tag writer App private key and must never be referenced by package-registry, provenance, or GitHub Release publish jobs. `production-evidence-write-<project-name>` is a separate evidence-persistence environment used only by `require-provenance`; it carries the release-evidence writer App private key and must never be referenced by package-registry, GitHub Release, or tag-reservation jobs. `control-plane-audit` is a repository-scoped protected environment used only by `preflight-check` and the privileged monitoring workflows that need the environment-reader or ruleset-auditor GitHub App credentials. The three project-scoped environments must use the same exact deployment branch set, the same required-reviewer protection profile, and no environment wait timer. Intentional release holds are modeled through checked-in control-plane state such as planned-change windows, not through environment wait timers, so approval-latency monitoring in this design measures only human review latency. GitHub Environments still treat required reviewers as a one-of-N approval pool rather than a dual-approval gate, so the at-least-two-reviewer requirement in this design is reviewer-pool redundancy, not a claim that GitHub will require two approvals. This keeps write credentials and audit credentials isolated by function rather than concentrating them in general publish jobs.
...
| Generate official build attestations in the single language-matching release build job | `id-token: write`, `attestations: write` |
...
Because GitHub attestations are bound to the job that generates them, official release-mode build jobs must hold `id-token: write` and `attestations: write` long enough to generate build-time attestation bundles for the exact artifacts they produced. `require-provenance` then downloads that artifact set plus the generated bundles, verifies the bundles against the expected build-job identity, and writes the durable evidence record. `require-provenance` must not mint a second attestation set for already-built artifacts.
...
**Job timeouts:** Every job must declare `timeout-minutes`, and workflow linting enforced through `hk`/`actionlint` should fail if any job omits it. Recommended defaults: `preflight-check`, resolution jobs, and static-analysis jobs `15`; Ubuntu build jobs `30`; Windows C# build jobs `45` because hosted Windows runners have materially higher startup and .NET restore/build/test overhead than Ubuntu runners; `require-provenance` `45` because it now downloads the full build artifact plus attestation bundles, verifies the attestation set, performs bounded evidence-branch append retries with immutable read-back verification, and must emit phase checkpoints for timeout diagnosis; publish jobs `25`; `confirm-publish-state` `25` because it performs bounded remote rechecks in parallel across the selected destinations and still needs time for runner startup, checkout, locked-tool bootstrap, checkpoint persistence, and output serialization; lightweight tag-management jobs `10`; and terminal gate jobs `ci-passed` and `release-complete` `10` because those jobs must perform a read-only checkout and bootstrap `jq` from the locked `mise` toolchain before running their jq assertions. Some YAML snippets below omit `timeout-minutes` only for brevity; concrete workflow files must still declare it.
...
4. **`trusted-release-inventory`**:
    ...
    - The same job must also validate `.github/release-recovery-ledger.jsonl` against the Section 7 schema whenever that file changes, including `schemaVersion`, strict key whitelists, `incidentId` UUID requirements, required `revision` monotonicity per `incidentId`, required `releaseLine`, the required incident and audit governance fields (`owner`, `severity`, `acknowledgedAt`, and `nextReviewAt` where applicable), closed sets, conditional `runAttempt`, `workflowRunUrl`, `evidenceUrl`, `artifactEvidenceUrl`, `closedAt`, `automationId`, `scriptVersion`, and `credentialId` rules, canonical target ordering, the `selectedTargets` / `publishedTargets` / `pendingTargets` partition invariants, the `deprecatedTargets`, `delistedTargets`, `removedTargets`, and `retainedTargets` subset invariants, the disposition-specific distinguishing rules for `open-before-publish`, `open-partial-publish`, `recovered`, `abandoned-before-publish`, `abandoned-after-partial-publish`, `burned`, and `fully-withdrawn`, exhaustive target accounting for terminal published states, and the presence-and-absence rules for the hold-window evidence fields used in destructive stable-release recovery.
...
- **Project-scoped publish environments:** For every releasable project, `environment: production-<project-name>` must exist in GitHub repository settings with protection rules that include required reviewers, `prevent_self_review = true`, `Allow administrators to bypass configured protection rules = false`, and no configured wait timer **before** the workflow is ever triggered. Because GitHub Environments require only one approval from the configured reviewer set, the design's at-least-two-reviewer rule is reviewer-pool redundancy rather than a claim of dual approval. If that environment does not pre-exist, GitHub auto-creates it with **zero** protection rules and the human approval gate silently does not exist.
- **Project-scoped tag-write environments:** For every releasable project, `environment: production-tag-write-<project-name>` must also exist before the workflow is ever triggered. It must use the same required-reviewer protection profile, the same disabled-admin-bypass setting, no configured wait timer, and the same deployment branch policy as `production-<project-name>`, and it is reserved exclusively for `create-release-tag` and the release-tag writer App private key.
...
- **Repository-scoped control-plane audit environment:** `environment: control-plane-audit` must exist before the first official run. It is reserved exclusively for `preflight-check` and the privileged monitoring workflows that need the environment-reader or ruleset-auditor GitHub App credentials. It must have required reviewers, `prevent_self_review = true`, administrator bypass disabled, no configured wait timer, and deployment branches limited to the protected control-plane branch set.
...
- **GitHub App credentials and explicit registry secrets:** Before first use, release engineering must provision four GitHub Apps and store their private keys in the narrowest possible GitHub secret scopes: an environment-reader App for `preflight-check` and control-plane monitors, a separate ruleset-auditor App for authoritative Repository Rulesets reads including `bypass_actors`, a release-tag writer App whose private key is stored only in the corresponding `production-tag-write-<project-name>` environment for `create-release-tag`, and a release-evidence writer App used only by `require-provenance` to push to `refs/heads/release-evidence`. The environment-reader and ruleset-auditor App private keys must be stored only in `control-plane-audit`; `production-<project-name>` must never carry either audit credential or either write credential. These Apps must request only the repository permissions required for their single purpose. The required scopes are strict: the environment-reader App must have `actions: read` plus the minimum documented read scope GitHub requires for environment and deployment-policy reads; the ruleset-auditor App must have the minimum GitHub App permission set that returns `bypass_actors` in the Repository Rulesets API, and because GitHub's public docs currently require write access to the ruleset for that field, this design isolates that broader read credential behind the separate `control-plane-audit` environment rather than exposing it to every workflow job; the release-tag writer App must have `contents: write` and no other repository scopes; the release-evidence writer App must have `contents: write` and no other repository scopes. Because NuGet.org does not have documented trusted publishing in this design, each releasable project that publishes to `nuget:official` must also carry exactly one explicit `NUGET_API_KEY` secret in `production-<project-name>`, and that secret must be passed only to `publish-nuget-official`. The provider-side key scope must be limited to the exact package ID or project-specific package pattern for that one project; cross-project NuGet.org push keys are unsupported. Organization administrators remain the trusted root for these GitHub-hosted secrets; this design does not attempt to defend against org-admin compromise. All private keys and explicit registry secrets must rotate at least every 90 days and immediately on suspected compromise, and the revocation runbook must define how to revoke the old key or API key, install the replacement, validate the tag-write path, evidence-write path, audit path, or NuGet publish path as applicable, review GitHub audit logs for abuse, and audit the protected `refs/tags/release/**` namespace plus the `release-evidence` branch for unexpected mutation. Any GitHub App installation token minted at runtime must be masked immediately after issuance, before any other use, and should be explicitly revoked at job end on a best-effort basis in addition to relying on its native expiry.
...
- **Live control-plane drift detection:** In addition to near-real-time administration alerts, the repository must run a dedicated scheduled control-plane drift monitor at least every 30 minutes. This monitor is a named control-plane monitor with its own dual external dead-man's-switch heartbeats and must query every live `production-<project-name>`, `production-tag-write-<project-name>`, and `production-evidence-write-<project-name>` deployment branch policy, verify that none of those environments has a configured wait timer, compare the exact short-branch set to `.github/official-caller-refs.json`, verify that `.github/official-caller-refs.json` plus `.github/publish-trust-inventory.json` are byte-identical across every branch currently in the protected control-plane branch set, verify that `refs/heads/release-evidence` exists and is protected, verify the expected branch and tag ruleset profile, verify the authoritative `bypass_actors` set through the ruleset-auditor App, and verify that the required GitHub App installations are present. Outside a non-expired matching `active` or `quarantine` record in `.github/planned-change-windows.json`, any mismatch between live GitHub state and the checked-in caller-ref registry is a control-plane incident even if no release is currently running. `cooldown` records document recent changes but do not authorize live drift. Drift automation must also fail if `.github/planned-change-windows.json` contains an expired window that has not been removed, and it must emit pre-expiry alerts for planned-change windows at least 24 hours before `quarantine` expiry and at least 60 minutes plus 15 minutes before any `active` window expiry.
...
- **Shared trusted-publisher boundary note:** No registry in scope publicly documents stronger isolation between multiple jobs that run inside the same entry workflow and the same protected environment. This design therefore treats all official trusted-publisher jobs that execute inside `production-<project-name>` as one shared external trust boundary. The compensating control is that those jobs, their checked-in execution paths, and their helper code all remain on the protected control-plane branch set under mandatory review; the design does not claim registry-side per-job separation that the providers do not document.
...
```json
{
    ...
    "publishWorkflowPaths": {
        ...
        "npm:official": ".github/workflows/official.yml",
        "pypi:official": ".github/workflows/official.yml",
        "rubygems:official": ".github/workflows/official.yml",
...
No top-level keys other than `schemaVersion`, `entryWorkflowPath`, `allowedCallerRefs`, `publishWorkflowPaths`, `targetEnvironments`, `targetAuthMechanisms`, `targetTrustedPublisherSelectors`, and `targetOidcAudiences` are allowed. The validator must enforce this as a strict top-level key whitelist equivalent to JSON Schema with `additionalProperties: false`. `publishWorkflowPaths`, `targetEnvironments`, and `targetAuthMechanisms` must contain exactly the five official target keys. `targetTrustedPublisherSelectors` must contain exactly the currently trusted-publisher-backed targets `{npm:official, pypi:official, rubygems:official}` and no others. Every selector object must contain exactly `providerContract`, `repositoryOwner`, `repositoryName`, `workflowPath`, and `environment`. `providerContract` must use the closed set `{entry-workflow-ui}` in this design. `targetOidcAudiences` must contain exactly the targets whose provider publicly documents a required audience in this design; RubyGems.org is intentionally absent because the public provider documentation reviewed for this design does not publish a required audience value for its GitHub Actions trusted-publishing flow. As of v2.6 the documented-audience set is `{npm:official, pypi:official}`.
...
1. **`preflight-check`**:
    ...
    - This job must not perform repository checkout. It must run inside `environment: control-plane-audit` and mint two dedicated GitHub App installation tokens just in time: an environment-reader token for GitHub Environments and deployment branch policy reads, and a separate ruleset-auditor token for authoritative Repository Rulesets reads including `bypass_actors`. A long-lived PAT is not the normal path. The GitHub App private keys for these audit tokens must live only in `control-plane-audit`, not in organization-level or repository-level general workflow secrets. Treat the default `GITHUB_TOKEN` as insufficient for this job; weakening or skipping the verification is unsupported. Every minted installation token must be masked before first use.
    ...
    - Verifies that all three derived environments already exist, each includes a required-reviewer protection rule with at least two reviewers total, each has `prevent_self_review` enabled, each has administrator bypass disabled, none of them has a configured wait timer, each restricts deployment branches to the official protected control-plane branch set for that project, and all three environments expose the same reviewer pool and the same branch-policy set.
...
4. **`clean-build`** (`build-csharp` / `build-python` / `build-jsts` / `build-ruby`):
    ...
    - Uses the same four static conditional build jobs pattern as `buddy.yml`. In `build-scope: release` mode, the single language-matching build job must request `permissions: { contents: read, id-token: write, attestations: write }`, use `secrets: {}`, and receive the required `with:` inputs wired from `build-scope: release`, `needs.resolve-context.outputs.project-path`, `needs.resolve-context.outputs.project-name`, `checkout-ref: ${{ github.sha }}`, and `require-provenance: true`. Each build job depends on both `resolve-context` and `static-analysis`. Only the language-matching build job executes; the others are skipped. The matching release-mode build job must generate the attestation bundle set for the exact manifest-selected publication files so the attested signer is the build job itself.
...
7. **Publish jobs** (static conditional, one job per official ecosystem-destination pair):
    ...
    - `publish-npm-official`, `publish-pypi-official`, and `publish-rubygems-official` use `permissions: { contents: read, id-token: write }` for trusted publishing and run directly inside `official.yml`. They may call reviewed composite actions or helper scripts for shared artifact-validation or upload logic, but they must not call local reusable workflows because the external trusted-publisher registration is intentionally bound to `.github/workflows/official.yml`. `publish-nuget-official` uses `permissions: { contents: read }` plus the single explicit `NUGET_API_KEY` secret from `production-<project-name>`. `publish-github-official` uses `permissions: { contents: write }`, which already satisfies read access.
    - Each direct trusted-publisher job must expose job-level outputs for the downstream terminal gate. At minimum, `publish-npm-official`, `publish-pypi-official`, and `publish-rubygems-official` must each map a composite-action step output `publish-result` to `jobs.<id>.outputs.publish-result`, and `publish-npm-official` must also map `applied-dist-tags` to `jobs.<id>.outputs.applied-dist-tags`. `release-complete` consumes those job outputs; step summaries alone are insufficient.
...
| `require-provenance` | `boolean` | No | When `true`, the release-mode build workflow must emit the deterministic build-side metadata consumed later by `require-provenance`, including the build-generated attestation bundles and `attestation-manifest.json`; defaults to `false` and is valid only when `build-scope = release` |
...
When `require-provenance = true`, the build workflow must place `build-verification-input.json`, `attestation-manifest.json`, and the `attestations/` bundle directory in the main build artifact next to `artifact-manifest.json`. The build workflow generates those attestation bundles as part of the release-mode build so the attested signer is the build job itself. `artifact-evidence.json` is not part of the build-produced artifact; it is produced only later by `require-provenance` after verification succeeds. `build-verification-input.json` is internal control-plane metadata rather than a GitHub Release asset and uses a fixed exact schema:
...
No top-level keys other than those shown above are allowed. `schemaVersion` must equal `1`. `artifactManifestSha256` must match exactly `[0-9a-f]{64}`. `buildJobWorkflowRef` and `buildWorkflowSha` must be the exact values later verified from the build-generated attestation bundles. This file records only deterministic build-side facts and must never contain any `verified*` claim or any run-unique URL.

The build workflow must generate `attestation-manifest.json` using this exact schema:
...
Official trusted-publisher-backed jobs (`publish-npm-official`, `publish-pypi-official`, and `publish-rubygems-official`) are ordinary jobs in `official.yml`, not reusable workflows. The reusable publish contracts below apply to buddy GPR publishes plus the non-trusted-publisher official surfaces that remain reusable (`_publish-nuget.yml` and `_publish-github.yml`).
...
**Secrets:** `secrets: {}` by default. `publish-nuget-official` is the one exception in this design and must receive exactly one named secret `NUGET_API_KEY` from `production-<project-name>`. Reusable publish workflows must never rely on trusted publishing in this design. `secrets: inherit` is prohibited.
...
For GitHub Releases, remote identity comparison is role-filtered and metadata-aware: the expected asset set is exactly the manifest entries whose `publishRoles` include `github-release-asset` plus the deterministic `SHA256SUMS` file derived from that same set; internal workflow metadata such as `artifact-manifest.json` is never part of that comparison. A rerun may repair a strict remote subset state when every already-present remote asset, including `SHA256SUMS`, matches the current local digest set and only expected assets are missing; any mismatched existing asset digest is a hard failure.
...
Operators must also treat GitHub's lifetime limits as first-class recovery boundaries. GitHub's documented 30-day workflow-rerun limit and the recommended 90-day official artifact retention are distinct timers with different failure modes. GitHub does not publicly document a separate approval-request expiry, so this design tracks long-lived pending approvals against the enclosing run's rerun deadline instead of inventing an independent GitHub expiry. If a run itself expires while approvals are still pending, audit the resulting `release/**` tag state against completed `release-complete` runs before choosing recovery, even if artifacts are still retained. If artifacts later expire as well, the original run is no longer recoverable and the same-identity rebuild rule from step 10 becomes the only supported path from the still-unchanged protected branch. The repository must maintain seven control-plane monitors in addition to the reviewed ledger updates. First, a scheduled control-plane drift monitor must run at least every 30 minutes, verify live deployment policies plus cross-branch byte-identical control-plane files, verify that the `control-plane-audit` environment remains present and bypass-disabled, and ping an external dead-man's-switch heartbeat on success. Second, a scheduled approval-age monitor must run at least every 5 minutes, enumerate official runs still waiting on `production-evidence-write-*`, `production-tag-write-*`, or `production-*` approvals, measure age from the GitHub environment-review request timestamp rather than from monitor detection time, alert immediately on first detection, then re-alert at 10 minutes without acknowledgement, escalate at 30 minutes without an incident plan, escalate to engineering management at 60 minutes, and require an explicit continuation grant or cancellation decision by 90 minutes; when no owning incident exists yet, it must open or update the corresponding `open-before-publish` incident before the first re-alert, and it must ping an external dead-man's-switch heartbeat on success so the repository can detect when the monitor itself is disabled or stalled. Third, an event-driven post-tag failure monitor must react whenever an official run reaches `create-release-tag` success with `tag-result` in `{created, no-op}` for the expected `release/<project-name>/v<version>` identity and later settles in a non-success conclusion, then open or update the tracked recovery issue and require a ledger reconciliation check instead of relying on operators to notice the failed run manually. Fourth, a scheduled post-tag-monitor health check must run at least every 24 hours, inspect recent official runs for any missing event-driven incident creation after a qualifying post-tag non-success conclusion, backfill the recovery issue when necessary, and ping its own external dead-man's-switch heartbeat on success so loss of the event trigger path is detectable. Fifth, an open-incident freshness monitor must run at least every 6 hours, re-query every open `incident` ledger entry with `disposition` in `{open-before-publish, open-partial-publish}`, compare live GitHub Release and registry state against the stored `publishedTargets`, `pendingTargets`, `tagState`, and `githubReleaseState`, verify that `owner`, `severity`, and `nextReviewAt` remain populated, warn once an incident reaches 7 days, escalate to the release-engineering owners at 14 days, escalate to engineering management at 21 days, alert when `eligibleDeleteAt` falls within the next 24 hours, and automatically open or update a tracked follow-up issue if any such incident remains unresolved at 30 days. Sixth, a 7-day operational audit must enumerate protected `release/**` tags, enumerate GitHub Releases whose deterministic stable title matches `<project-name> v<version>`, confirm each one corresponds either to a completed official release or to an explicitly tracked burned or fully withdrawn identity under the recovery policy, and record that audit outcome in `.github/release-recovery-ledger.jsonl`. That 7-day audit must also enumerate every open `incident` ledger entry with `disposition` in `{open-before-publish, open-partial-publish}`, re-query the live GitHub Release state plus every selected remote registry target, and compare those live results against the stored `publishedTargets`, `pendingTargets`, `tagState`, and `githubReleaseState`; point-in-time ledger snapshots are not sufficient for open incidents. `full-release-tag-namespace` audits are the default periodic sweep and must enumerate every protected `release/**` tag. `project-release-line` audits are for scoped follow-up on a single protected branch or release line after a localized incident. `single-release-identity` audits are for one deterministic tag or version identity during active recovery. `emergency-cleanup-governance` audits are governance attestations for the break-glass group and do not enumerate release tags. `control-plane-audit` audits are repository-scoped checks over the audit Apps, environment protections, and monitor-heartbeat credentials themselves. Seventh, a daily governance-and-runbook freshness monitor must verify that the newest emergency-cleanup governance attestation is not older than 30 days and that the newest withdrawal-runbook and registry-auth-rollback-runbook re-attestations are not older than 90 days, then ping its own external dead-man's-switch heartbeat on success. The audit requirement must be enforced by a scheduled control-plane audit workflow, and that workflow must ping an external dead-man's-switch heartbeat on success so the repository can alert when the audit workflow itself fails, is disabled, or otherwise stops running. The external dead-man's-switch contract is strict: use a provider that accepts authenticated HTTPS heartbeats, configure one check for the drift monitor with an expected interval of 2 hours, one check for the approval-age monitor with an expected interval of 30 minutes, one check for the post-tag-monitor health check with an expected interval of 60 hours, one check for the open-incident freshness monitor with an expected interval of 24 hours, one check for the 7-day audit with an expected interval of 15 days, and one check for the governance-and-runbook freshness monitor with an expected interval of 60 hours, and alert after the first missed interval for each. Those thresholds intentionally allow one missed scheduled execution plus normal scheduler jitter before paging, rather than paging on the first slightly late run. A second repository-side watchdog may alert on the age of the newest successful audit run, but it is only a supplement to the external heartbeat, not a replacement. This automation may automatically append `audit` ledger records when either `result = clean` with `followUpStatus = not-required`, or `result = discrepancy-found` with `followUpStatus = required-open` and a linked follow-up issue created or referenced in the same operation; reconciliations, break-glass usage, and incident-state changes still require a reviewed ledger update. Any `discrepancy-found` audit result that is not reconciled during the same audit must create or link a tracked follow-up issue before the audit is considered complete, must set `followUpStatus = required-open`, and must either be reconciled or escalated to the release-engineering owners within 24 hours. A later ledger entry may mark `followUpStatus = resolved` only after the discrepancy has been reconciled and the closing audit evidence is recorded.
