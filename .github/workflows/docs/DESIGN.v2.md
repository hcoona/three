...
2. **Freeze release traffic.**
   ...
   - complete the Day -1 legacy-automation clearance checklist on the branch being migrated:
     ...
     - every remaining `pull_request_target` workflow is metadata-only; repository-maintenance workflows such as `dependabot-auto-merge.yml`, when present, must either remain on `pull_request` or satisfy the metadata-only rule before official enablement
