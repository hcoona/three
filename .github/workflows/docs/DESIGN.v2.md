...
4. **`trusted-release-inventory`**:
    ...
    - The same job must also validate `.github/planned-change-windows.json` against the Section 4 schema whenever that file changes, including `schemaVersion`, the strict key whitelists, unique `windowId` enforcement, operation/status closed sets, RFC 3339 UTC timestamps, immutable `openedAt`, immutable `closedAt` once present, required `phaseStartedAt`, per-operation and per-status maximum phase duration, conditional `linkedIncidentId`, `drainAttempts`, `closedAt`, and `cooldownUntil` rules, the mandatory 48-hour cooldown before the same `{operation, projectName, releaseLine}` tuple may open a new active window, and cross-file referential integrity that every `linkedIncidentId` points to an open `open-before-publish` ledger entry for the same `{projectName, releaseLine}`.
    - The same job must also validate `.github/release-recovery-ledger.jsonl` against the Section 7 schema whenever that file changes, including `schemaVersion`, strict key whitelists, `incidentId` UUID requirements, closed sets, conditional `runAttempt`, `workflowRunUrl`, `evidenceUrl`, `artifactEvidenceUrl`, `closedAt`, `automationId`, and `scriptVersion` rules, canonical target ordering, the `selectedTargets` / `publishedTargets` / `pendingTargets` partition invariants, the `withdrawnTargets` and `retainedTargets` subset invariants, the disposition-specific non-empty-set rules for `open-before-publish`, `open-partial-publish`, `recovered`, `abandoned-after-partial-publish`, `burned`, and `fully-withdrawn`, and the presence-and-absence rules for the hold-window evidence fields used in destructive stable-release recovery.
...
No top-level keys other than `schemaVersion` and `windows` are allowed. `windows` must be an array of unique objects where `windowId` is unique. Every window object must contain exactly `windowId`, `operation`, `status`, `projectName`, `releaseLine`, immutable `openedAt`, `phaseStartedAt`, `expiresAt`, and `openedBy`, plus optional `linkedIncidentId`, `drainAttempts`, `closedAt`, `cooldownUntil`, and `notes`. `operation` must use the closed set `{onboarding, retirement}`. `status` must use the closed set `{active, quarantine, cooldown}`. `openedAt`, `phaseStartedAt`, `expiresAt`, `closedAt`, and `cooldownUntil` must be RFC 3339 UTC timestamps. `projectName` and `releaseLine` must each match the same validation rules used elsewhere in this design. `openedAt` records when the window tuple was first opened and must never be rewritten; `phaseStartedAt` records when the current status phase began; `closedAt`, once present, must never be rewritten. When `status = active` and `operation = onboarding`, `expiresAt - phaseStartedAt` must be no more than 24 hours, `linkedIncidentId`, `drainAttempts`, `closedAt`, and `cooldownUntil` must be absent, and a reviewed active-to-active phase extension is valid only if it keeps `openedAt` unchanged, rewrites `phaseStartedAt` plus `expiresAt`, and records the extension rationale in `notes`. When `status = active` and `operation = retirement`, `expiresAt - phaseStartedAt` must be no more than 8 hours, `linkedIncidentId`, `closedAt`, and `cooldownUntil` must be absent, and `drainAttempts` is required as a non-negative integer. When `status = quarantine`, `operation` must be `retirement`, `expiresAt - phaseStartedAt` must be no more than 7 days, `linkedIncidentId` is required and must equal the exact open incident record's `incidentId`, `drainAttempts` is required, and `closedAt` plus `cooldownUntil` must be absent. A transition from `active` to `quarantine` must keep `openedAt` unchanged, must rewrite `phaseStartedAt` and `expiresAt` to the quarantine phase's start and end timestamps in the same reviewed change, and is valid only for an `open-before-publish` incident. A transition from `quarantine` back to `active` is valid only after that linked incident is closed, must clear `linkedIncidentId`, must keep `openedAt` and `drainAttempts` unchanged, and must rewrite `phaseStartedAt` and `expiresAt` for the resumed retirement phase. When `status = cooldown`, `closedAt` and `cooldownUntil` are required, `cooldownUntil` must be at least 48 hours after `closedAt`, and the record must be removed promptly once `cooldownUntil` passes. `cooldown` authorizes only the exact settled post-change state implied by `operation`: for `onboarding`, the branch must already be present in all three protected environments and in the checked-in caller-ref and publish-trust inventories; for `retirement`, the branch must already be absent from those same locations. At most one non-expired window may exist for the same `{operation, projectName, releaseLine}` tuple, and a new `active` window for that tuple is forbidden until the prior record's cooldown has expired and been removed.
...
1. **`preflight-check`**:
    ...
    - Verifies that all three derived environments already exist, each includes a required-reviewer protection rule with at least two reviewers total, each has `prevent_self_review` enabled, and each restricts deployment branches to the official protected control-plane branch set for that project.
    - Uses the GitHub Environments API response directly: the check must look for a `protection_rules` entry with `type == "required_reviewers"` and a reviewer list whose total length is at least `2`, must verify `prevent_self_review == true`, and must verify that the deployment branch policy on all three environments contains only exact branch names for `main` plus the registered protected maintenance branches for that project. Wildcard or pattern-based entries such as `release/**` are hard failures. Because the API returns short branch names rather than fully qualified refs, this job must normalize the expected caller refs by stripping the `refs/heads/` prefix before comparison. A wait timer or branch policy alone is not sufficient. This job verifies the protection quality of every branch already listed in those deployment policies; completeness of the allowed caller-ref set is enforced separately by the publish trust inventory preflight in `resolve-context`, and `refs/heads/release-evidence` is outside that environment-derived branch set by design.
...
5. **`require-provenance`**:
    ...
    - Downloads the main build artifact plus the provenance artifact emitted by the single language-matching build job, validates `build-verification-input.json`, validates `attestation-manifest.json`, verifies the deterministic `attestations/` layout, recomputes the SHA-256 of the downloaded `artifact-manifest.json`, and verifies binding to the exact workflow run, repository identity, source commit, project identity, version, and build artifact manifest for this release attempt. The validation is exact: every manifest entry selected for publication must have exactly one attestation-manifest entry and exactly one matching bundle file, and no extra attestation-manifest entry or extra bundle file is allowed.
...
When `require-provenance = true`, the build workflow must also upload a second CI artifact named `build-provenance-<project-name>` and expose that name via `provenance-artifact-name`. That artifact must contain exactly `build-verification-input.json`, `attestation-manifest.json`, and one Sigstore bundle file per attested manifest entry under `attestations/`. `artifact-evidence.json` is not part of the build-produced provenance artifact; it is produced only later by `require-provenance` after verification succeeds. `build-verification-input.json` is internal control-plane metadata rather than a GitHub Release asset and uses a fixed exact schema:
...
`attestation-manifest.json` is also mandatory and uses this exact schema:
