...
public sealed class CloudflareZoneResolverTests
{
    ...
    public async Task ResolveAsyncWalksSuffixesUntilItFindsAnExactMatch()
    {
        ...
        CloudflareZone zone = await resolver.ResolveAsync("API.Example.Com.", CancellationToken.None);
    ...
    {
        ScriptedHttpMessageHandler handler = new([
            () => JsonResponse("""{"success":true,"result":[{"id":"zone-1","name":"example.com"},{"id":"zone-2","name":"example.com"}],"errors":[],"messages":[],"result_info":{"page":1,"per_page":20,"count":2,"total_count":2,"total_pages":1}}"""),
        ]);
    ...
    private static CloudflareZoneResolver CreateResolver(ScriptedHttpMessageHandler handler)
    {
        ...
        CloudflareApiClient apiClient = new(client, new CloudflareConfiguration("token", [], false));
