...
public sealed class VsCodeSettingsManagerTests
{
    ...
    [Fact]
    public void RollbackWritePlanRestoresUnixFileAndDirectoryModes()
    {
        ...
        try
        {
            ...
            string managedHookFilePath = Path.Combine(
                tempDirectory.FullName,
                AppConstants.ManagedHookFileName);
            UnixFileMode originalDirectoryMode =
            ...
            UnixFileMode originalFileMode =
                ...
                | UnixFileMode.GroupRead;
            File.WriteAllText(settingsPath, """{ "editor.fontSize": 14 }""");
            File.SetUnixFileMode(tempDirectory.FullName, originalDirectoryMode);
            ...
            Assert.True(applyResult.Applied);
            Assert.True(rollbackResult.Applied);
            Assert.Equal(originalFileMode, File.GetUnixFileMode(settingsPath));
            Assert.Equal(originalDirectoryMode, File.GetUnixFileMode(tempDirectory.FullName));
        }
        finally
        {
            tempDirectory.Delete(recursive: true);
    ...
    [Fact]
    public void RollbackWritePlanRemovesDirectoriesCreatedForAbsentSettingsFile()
    {
        DirectoryInfo tempDirectory = CreateHomeScopedTempSubdirectory();

        try
        {
            string createdDirectoryPath = Path.Combine(tempDirectory.FullName, "new", "nested");
            string settingsPath = Path.Combine(createdDirectoryPath, "settings.json");
            string managedHookFilePath = Path.Combine(
                tempDirectory.FullName,
                AppConstants.ManagedHookFileName);
            ConfigurationPlanResult plan = VsCodeSettingsManager.PlanRegisterHookFile(
            ...
            Assert.True(applyResult.Applied);
            Assert.True(rollbackResult.Applied);
            Assert.False(Directory.Exists(Path.Combine(tempDirectory.FullName, "new")));
        }
        finally
        {
            tempDirectory.Delete(recursive: true);
