...
- [Workflow Release Descriptor Schema](./analyses/workflow-release-descriptor-schema.md) | Author-time file inventory and YAML schema for project descriptors, shared target-instance catalog entries, field-scoped path rules, out-of-`src/` descriptor rejection, project-local variant-id handle uniqueness plus rejection of duplicate semantic variant `dimensions` sets, variant-local semantic artifact-identity uniqueness despite descriptor-local `artifact.id` handles, checked-in source-file resolution, closed current-scope ecosystem-to-primary-manifest mapping, current-scope contract, capability, projection, and artifact-compatibility rules, discovery, references, and validation boundaries. |
- [Workflow Release Plan Shape](./analyses/workflow-release-plan-shape.md) | Authoritative `three.release.plan/v1alpha1` planner output shape covering the top-level envelope and normalized graph, deterministic plan and graph id generation, normalized request flags inside `plan-id`, selected-project source snapshots, planner-authored resolved publish identity plus replay disposition and live publish mode, a closed immutable-target and `buddy FORCE` outcome matrix, normalized projection data, frozen target-instance snapshots, deterministic Group 1 mapping, and explicit out-of-plan boundaries. |
- [Workflow Release Workflow and Executor Boundaries](./analyses/workflow-release-workflow-executor-boundaries.md) | Control-plane entry-point and orchestration boundaries for consuming `three.release.plan/v1alpha1`, including normalization of the planner-facing request contract, one build unit per variant, one publish unit per publish node, explicit job-to-job artifact and receipt handoffs, executor input or output contracts, and explicit ownership of approvals, concurrency, tagging, runtime wiring, and reporting. |
