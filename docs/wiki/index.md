...
- [Workflow Release Low-Level Design](./analyses/workflow-release-low-level-design.md) | Lower-layer implementation handoff that freezes stable workflow filenames for trusted publishing, first-delivery live PyPI/RubyGems through reusable-workflow OIDC and npmjs through caller-workflow OIDC, entry inputs, deferred dry-run/validation-build behavior, planner diagnostic-code registration, JSON handoff files, artifact and immutable-proof naming, registry adapter obligations, GitHub permissions, tag orchestration, and acceptance traceability, dedicated GitHub Release smoke artifact coverage, and intentional Python app smoke deferral while leaving internal code decomposition implementation-owned. |
- [Workflow Release Low-Level Design Rebaseline Recommendation](./analyses/workflow-release-low-level-design-rebaseline-recommendation.md) | Process record for preserving low-level design investment during major middle-layer topology changes, documenting the first-class OIDC and live PyPI rebaseline and how to repeat or verify similar passes. |
- [Workflow Release OIDC Publish Topology Research](./analyses/workflow-release-oidc-publish-topology.md) | Registry trusted-publisher support matrix showing why external OIDC publish topology must become a first-class design dimension, why live PyPI publication belongs in first delivery under the superseding orchestrator-hosted split topology, and why broader PyPI multi-wheel support remains a separate deferred issue. |
- [Workflow Release Operator Rollout Runbook](./analyses/workflow-release-operator-rollout.md) | Final implementation-readiness runbook covering manual repository and registry setup, local and CI gates, staged activation to live validation, diagnostics, rollout guardrails, and when live testing should begin. |
- [Workflow Release CI Affected Validation Artifact Enumeration Experiment](./analyses/workflow-release-ci-affected-validation-artifact-enumeration-experiment.md) | Group 1 platform experiment covering artifact enumeration fields, instance counting, run-attempt separation, attempt-visible and attempt-scoped variable-length physical artifact names, and LLD impact for CI affected validation. |
- [Workflow Release CI Affected Validation Producer Identity Experiment](./analyses/workflow-release-ci-affected-validation-producer-identity-experiment.md) | Group 2 platform experiment covering producer/job identity, matrix and job-rerun identity stability, writer-observation feasibility, and LLD impact for trusted receipt writers. |
- [Workflow Release CI Affected Validation No-Authoritative-Plan Experiment](./analyses/workflow-release-ci-affected-validation-no-authoritative-plan-experiment.md) | Group 3 platform experiment covering missing-plan failure behavior, readable diagnostics, downstream skip or aggregate topology, and LLD impact for final required-check failure. |
- [Workflow Release CI Affected Validation Platform Spike Summary](./analyses/workflow-release-ci-affected-validation-platform-spike-summary.md) | Group 4 synthesis of the three platform experiments, recording run links, validated assumptions, design constraints, remaining risks, and go/no-go readiness before LLD implementation. |
- [Workflow Release CI Affected Validation Requirements](./analyses/workflow-release-ci-affected-validation-requirements.md) | Requirement baseline for CI affected validation, including scope selection, fail-closed behavior, no-publish boundaries, and acceptance expectations. |
- [Workflow Release CI Affected Validation High-Level Design](./analyses/workflow-release-ci-affected-validation-high-level-design.md) | High-level design for planner-owned validation policy, subject selection, and validation evidence boundaries. |
- [Workflow Release CI Affected Validation Middle-Level Design](./analyses/workflow-release-ci-affected-validation-middle-level-design.md) | Middle-layer contracts for logical work groups, execution handoff, batching flexibility, evidence semantics, and workflow boundaries. |
- [Workflow Release CI Affected Validation Low-Level Design](./analyses/workflow-release-ci-affected-validation-low-level-design.md) | Low-level execution-batch CI handoff defining workflow shape, bounded runner-family orchestration, evidence artifacts, authority checks, and acceptance traceability. |
- [Workflow Release CI Affected Validation Implementation Plan](./analyses/workflow-release-ci-affected-validation-implementation-plan.md) | Handoff plan and completion record covering grouped implementation history, review protocol, Group 1 authority hardening, Group 2 hosted acceptance evidence, final global closure, and non-blocking runtime follow-ups. |
