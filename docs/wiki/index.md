...
- [Workflow Release Low-Level Design](./analyses/workflow-release-low-level-design.md) | Lower-layer implementation handoff that freezes stable workflow filenames for trusted publishing, first-delivery live PyPI through `publish-topology: external-oidc-entry-workflow`, entry-hosted publish scheduling, entry inputs, dry-run validation-build behavior, planner diagnostic-code registration, JSON handoff files, artifact and immutable-proof naming, registry adapter obligations, GitHub permissions, tag orchestration, and acceptance traceability, dedicated GitHub Release smoke artifact coverage, and intentional Python app smoke deferral while leaving internal code decomposition implementation-owned. |
...
- [Workflow Release OIDC Publish Topology Research](./analyses/workflow-release-oidc-publish-topology.md) | Registry trusted-publisher support matrix showing why external OIDC publish topology must become a first-class design dimension, why live PyPI publication belongs in first delivery, and why broader PyPI multi-wheel support remains a separate deferred issue. |
- [Workflow Release Operator Rollout Runbook](./analyses/workflow-release-operator-rollout.md) | Final implementation-readiness runbook covering manual repository and registry setup, local and CI gates, staged dry-run to live validation, diagnostics, rollout guardrails, and when live testing should begin. |
