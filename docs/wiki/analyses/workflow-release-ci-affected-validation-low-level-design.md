...
from descriptor or obligation metadata alone. Public batch bundles reject
`reused-validation-receipt` unless it is revalidated and byte-bound with
equivalent digest proof; the current public source proof is no-publish
validation output. If no real no-publish release build output can bind the
observed SHA-256 digests to produced artifact bytes, the selector emits a blocking
`artifact-shape-unconfirmed` result instead of a validation-only success.
