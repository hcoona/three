...
plans: they use an available subject universe with `subjects: []` and an
available fact snapshot with `providers: []`, both digest-bound by their companion
snapshot artifacts, to prove that no subject or provider fact input was needed.
Fail-closed plans may use `unavailable` with `id: null`, but diagnostics must
identify which snapshot could not be produced or confirmed and why. Aggregation
must reject structurally invalid plans instead of converting them into successful
inspectable fail-closed evidence.
