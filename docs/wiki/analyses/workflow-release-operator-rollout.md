...
| NuGet.org       | Keep live external NuGet.org disabled unless a successor design explicitly enables it. If enabled later, configure the conservative entry workflow identity `release-official.yml` with environment `release`.                                                        |
