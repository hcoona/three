...
a workflow or job boundary where that value is already available. The logical
group identity is `three-release:<entry-workflow-file>:<commit-sha>`, where the
entry workflow file is the selected top-level entry workflow such as
`release-buddy.yml` or `release-official.yml`. That group is held by exactly one
top-level entry continuation boundary that starts after commit resolution and
does not complete until shared orchestration, any entry-hosted publish jobs, and
final reporting are complete. Child reusable workflows, matrix rows, publish
jobs, and report jobs must not declare that same concurrency group themselves;
they are serialized by their dependency on the single lock-holding entry
continuation. This avoids GitHub Actions' same-group pending-run cancellation and
unspecified ordering semantics while still preventing a second same-entry
same-commit release continuation from overlapping live side effects. Current
scope defines no additional release concurrency key; adding a raw-ref,
project-subset, or other coarser key requires a successor design that analyzes
GitHub Actions pending-run cancellation and ordering semantics.
