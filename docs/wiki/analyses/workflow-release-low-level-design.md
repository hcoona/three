...
The representative ecosystem and artifact-shape rows below remain acceptance
anchors, but they do not narrow the descriptor authoring batch.

| Coverage category                         | Project                                             | Descriptor root                                                     | Primary manifest or build entry point                          | Required descriptor and handoff coverage                                                                                                                                                                           |
| ----------------------------------------- | --------------------------------------------------- | ------------------------------------------------------------------- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| C# packable library                       | `CircularList`                                      | `src/public/lib/CircularList/`                                      | `CircularList.csproj`                                          | Descriptor declares both profiles and models the package/release targets selected for this project.                                                                                                                |
...
| C# packable library                       | `Memoization`                                       | `src/public/lib/Memoization/`                                       | `Memoization.csproj`                                           | Descriptor declares both profiles and models the package/release targets selected for this project.                                                                                                                |
| C# public helper or generator             | `Memoization.Generators`                            | `src/public/lib/Memoization.Generators/`                            | `Memoization.Generators.csproj`                                | Descriptor declares both profiles; zero-target profiles are valid when the helper has no current publish target.                                                                                                   |
| C# packable library                       | `MicrosoftExtensions.Logging.MSTest`                | `src/public/lib/MicrosoftExtensions.Logging.MSTest/`                | `MicrosoftExtensions.Logging.MSTest.csproj`                    | Descriptor declares both profiles and models the package/release targets selected for this project.                                                                                                                |
| C# packable library                       | `MicrosoftExtensions.Logging.Xunit`                 | `src/public/lib/MicrosoftExtensions.Logging.Xunit/`                 | `MicrosoftExtensions.Logging.Xunit.csproj`                     | Descriptor declares both profiles and models the package/release targets selected for this project.                                                                                                                |
| C# packable library                       | `MicrosoftExtensions.Options.DedupChangeExtensions` | `src/public/lib/MicrosoftExtensions.Options.DedupChangeExtensions/` | `MicrosoftExtensions.Options.DedupChangeExtensions.csproj`     | Descriptor declares both profiles and models the package/release targets selected for this project.                                                                                                                |
| C# packable library                       | `PhiFailureDetector`                                | `src/public/lib/PhiFailureDetector/`                                | `PhiFailureDetector.csproj`                                    | Descriptor declares both profiles and models the package/release targets selected for this project.                                                                                                                |
| C# packable library                       | `WebHdfs.Extensions.FileProviders`                  | `src/public/lib/WebHdfs.Extensions.FileProviders/`                  | `WebHdfs.Extensions.FileProviders.csproj`                      | Descriptor declares both profiles and models the package/release targets selected for this project.                                                                                                                |
...
| C# public app binary                      | `PhiFailureDetector.Console`                        | `src/public/app/PhiFailureDetector.Console/`                        | `PhiFailureDetector.ConsoleApp.csproj`                         | Descriptor declares both profiles and models the app binary or zero-target profile state selected for this project.                                                                                                |
...
| C# private app binary                     | `vscode-copilot-telegram-hook`                      | `src/private/app/vscode-copilot-telegram-hook/`                     | `VSCodeCopilotTelegramHook.csproj`                             | Nonpackable app descriptor coverage for both profiles; target declarations follow the same app binary and GitHub Release rules as other in-scope app descriptors.                                                  |
...
| Python public app                         | `markdown-hybrid-search-mcp`                        | `src/public/app/markdown-hybrid-search-mcp/`                        | `pyproject.toml`                                               | Descriptor declares both profiles; zero-target profiles are valid when project metadata intentionally has no current upload target.                                                                                |
...
| Node public package or extension artifact | `steam-account-history-to-csv`                      | `src/public/lib/steam-account-history-to-csv/`                      | `package.json`                                                 | Descriptor declares both profiles; zero-target or archive-style GitHub Release modeling is explicit rather than inferred from package metadata.                                                                    |
...
| GitHub Release tag atomicity                                | Any GitHub Release fixture above                                                                                                                                                                                                                | Descriptor models GitHub Release target and release tag identity                                                                                                                                                                                                                                               | Plan freezes required tag set, selected commit SHA, create-only or replace-authoritative disposition, and asset set                                                                                                                                                                                                                     | `selected-github-release-publish-node-ids` and `active-github-release-publish-node-ids` identify the live GitHub Release nodes that require tag orchestration before publish                              | `tag-result.json` verifies all existing tags before creating missing tags; no publish request exists when any existing tag points elsewhere                                                                                                                                                                                                                                                           | GitHub Release observation matches tag and final asset state; no retargeted tag is accepted                                                                                                                                                                                                                                                                                | Success only when tag state is atomic and consistent; wrong existing tag fails before release mutation                                                                                                     |
