...
| Dispatch SHA pinning      | Resolve the UI-selected branch or tag before orchestration, pass the peeled commit SHA forward, and keep later stages pinned to that SHA.                                                 |
...
- `buddy` and `official` must not publish the same package name to the same
  registry, except for the narrow GitHub Packages smoke targets explicitly
  marked `same-name-allowed`. In the current smoke matrix, the both-profile
  same-registry exception is limited to the dedicated GitHub Packages smoke
  target; NuGet, npm, and RubyGems ecosystem smoke projects publish to GitHub
  Packages in `buddy` and to their public registries in `official`.
- Prior old-design GitHub Packages acceptance evidence is not sufficient for
  the new same-identity `buddy`/`official` GitHub Packages smoke path. After the
  implementing commit lands, acceptance must include fresh `buddy` GitHub
  Packages smoke publication evidence before the GitHub Packages path is
  carried forward.
...
| --------------------------------------------- | --------------------- | -------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `.github/workflows/buddy.yml`         | `workflow_dispatch`   | `buddy` entry workflow.                                                    | Stable entry contract; not a first-delivery trusted-publisher filename.                       |
| `.github/workflows/official.yml`      | `workflow_dispatch`   | `official` entry workflow, including early actor-permission authorization. | Trusted-publisher filename for first-delivery entry-hosted external registries.               |
...
applies only to `official.yml` and `release-publish-node.yml`.
...
`official.yml` for entry-hosted external publication and
`release-publish-node.yml` for the reusable-workflow-bound RubyGems.org
publication topology. `buddy.yml`, `release-orchestrate.yml`, and
...
workflow filename `official.yml`, and GitHub Actions environment
...
npmjs trusted publishing stores a GitHub Actions workflow filename and, when a
`workflow_call` chain is used to run `npm publish`, npm's documented
troubleshooting behavior validates the calling workflow name instead of the
workflow file that contains the publish command. First delivery therefore treats
npmjs as entry-hosted: configure npmjs to trust `official.yml` with the
`release` environment, and run the OIDC-token-requesting `npm publish` job in the
top-level official entry workflow. The first-delivery npmjs path must not route
the live trusted-publishing credential exchange through `release-orchestrate.yml`
or `release-publish-node.yml`. Shared scripts, composite actions, or libraries
may still be reused as long as the token request and live upload stay in
`official.yml`.
...
The active caller chain is the `official.yml` caller job invoking
...
5. Keep the `canary-override-non-public-ref` break-glass input defaulted to
   `false`; when `true`, allow only the dedicated `hcoona-release-smoke-*` canary projects
...
Current scope does not adopt native in-progress duplicate-run auto-cancellation. Each
top-level entry workflow must define workflow-level GitHub Actions concurrency
with `cancel-in-progress: false` and a literal group for that entry workflow:

- `buddy.yml`: `three-release:buddy`;
- `official.yml`: `three-release:official`.

This conservative first-delivery key is intentionally coarser than the selected
ref, resolved `commit-sha`, project subset, and other entry inputs. It serializes
the running live release window for the same entry workflow from run creation
through final completion, so the protected window naturally covers commit
resolution, shared orchestration, entry-hosted publish jobs, and final reporting
without requiring a post-resolution job-level lock. GitHub Actions native
concurrency is not a FIFO queue: at most one run and one pending run may exist per
group, and a newly queued same-entry run replaces any older pending run in that
group. Current scope accepts that platform pending-replacement behavior before
side effects start; it is reported as ordinary GitHub cancellation and is not a
repo-defined supersession protocol for an already running release.
...
entry workflow run's workflow-level concurrency and by normal job dependencies.
Current scope defines no SHA-keyed, raw-ref-keyed, project-subset-keyed, or
explicit external lock/queue release concurrency mechanism. Adding a finer-grained
key or explicit lock/queue requires a successor design that analyzes GitHub
...
that do not produce or publish NuGet-shaped package artifacts. `dotnet-metadata`
must not rediscover descriptors, accept additional project paths, reinterpret
target usage, recompute `requires-package-id`, or downgrade a descriptor
...
Windows helper must evaluate a non-empty MSBuild `PackageId` from the selected
commit's project manifest and include it as `package-id`. If the evaluated value
is empty, missing, or cannot be normalized, the helper or planner reports
...
upload, the same publish job that uploads the asset must attest the exact uploaded
bytes with `actions/attest@v4` before it emits a positive `publish-result.json`.
...
digest. The job requires `id-token: write`, `attestations: write`, and the normal
GitHub Release write permission already required for publication.
...
For the current `github-token` topology, GitHub Release publication and
`actions/attest` run in the reusable publish workflow, so the frozen value is
`hcoona/three/.github/workflows/release-publish-node.yml`. If a successor topology
...
The control plane also emits one
`release-github-release-asset-proof-v1-<safe-id(github-release-asset-binding-json)>-<run-id>-<attempt>`
artifact containing `github-release-asset-proof.json` for each successfully
uploaded and attested asset. The wrapper is positive evidence for the current run
and an optional lookup accelerator for later runs; the GitHub Artifact Attestation
service remains the authoritative proof source. If the wrapper artifact has
expired but the remote asset can still be downloaded and its attestation verifies,
the planner may still classify the asset as content-equivalent. If the
attestation has been deleted, cannot be fetched, or fails verification, proof is
unavailable.
...
5. require the downloaded byte size to equal the remote asset size reported by the
   GitHub Release API and any unexpired matching
   `github-release-asset-proof.json` wrapper if such wrappers are present;
...
official PyPI and npmjs publication use this path.
...
1. `official.yml` or `buddy.yml` resolves the trusted dispatch
...
For first-delivery PyPI and npmjs official publication, the job that requests the
registry trusted-publishing OIDC token and performs the live upload must be a job
in `.github/workflows/official.yml` with the GitHub Actions environment
...
reusable workflow call whose called workflow mints the registry OIDC token.
Internal code reuse is allowed through repository scripts, libraries, or
composite actions shared with other publish paths, provided the PyPI or npmjs
OIDC token is minted only by the top-level official entry workflow job.
...
- First delivery includes live official PyPI publication. The PyPI trusted
  ...
  `official.yml`, and environment `release`.
...
- The `npm/npmjs` target requires `external-oidc-entry-workflow` in first
  delivery.
- npmjs validates the workflow filename configured on npmjs.com. Its documented
  `workflow_call` troubleshooting behavior says validation checks the calling
  workflow name instead of the workflow containing `npm publish`; the
  `official.yml` -> `release-orchestrate.yml` ->
  `release-publish-node.yml` nested chain would therefore make the configured
  workflow identity ambiguous for first-delivery npmjs.
- The concrete first-delivery `npm publish` command and OIDC token request must
  run in the top-level `official.yml` entry workflow under the
  `release` environment. It must not run in `release-orchestrate.yml`, must not
  run in `release-publish-node.yml`, and must not be implemented as a reusable
  workflow call whose called workflow mints the npmjs OIDC token. Internal code
  reuse is allowed through repository scripts, libraries, or composite actions.
...
- NuGet.org uses NuGet trusted publishing with GitHub Actions OIDC when a
  `nuget/nuget-org` official target is live-enabled. The current descriptor and
  code scope includes the dedicated NuGet smoke official target
  `hcoona-release-smoke-nuget` using `nuget/nuget-org`; live full-success remains
  externally blocked until NuGet.org trusted publishing and service-side
  enablement are configured. No long-lived NuGet API key is a current-scope
  credential.
- First delivery still defers real package NuGet.org official publication for
  `hjg-pngcs` until that package path, the trusted-publishing policy,
  live-enable token, and `.snupkg` observation behavior are documented and
  tested.

Required publish topology:

- The conservative NuGet.org topology is `external-oidc-entry-workflow` with
  `official.yml` and the `release` environment.
...
- Until the real package path is explicitly brought into scope and live-enabled,
  descriptors must not declare a `nuget/nuget-org` live target for first-delivery
  `hjg-pngcs`; keep `.nupkg` and `.snupkg` artifacts modeled for GitHub Release
  evidence only. Real package-registry publication for `hjg-pngcs`, including
  NuGet.org and GitHub Packages package targets, remains separately deferred.
  This restriction does not disable the dedicated NuGet smoke descriptor that
  already targets `nuget/nuget-org`.
...
- The active permission chain is the `official.yml` caller job, the
...
- GitHub Release uses `GITHUB_TOKEN`; it has no external OIDC trusted-publisher
  ...
  to obtain the Sigstore signing certificate for `actions/attest`; that token is
  for the attestation proof path, not for GitHub Release mutation authority.
...
- `external-oidc-entry-workflow`: grant `id-token: write` only to the
  ...
  and performs the upload. PyPI and npmjs use this path in first delivery;
  NuGet.org stays on this conservative path until registry behavior is verified
  and this design is updated. Reusable orchestration, planning, build, tag,
  report, skip, GitHub Release, and GitHub Packages jobs must not receive this
  OIDC grant for that publish node.
- `external-oidc-caller-workflow`: grant the OIDC capability along the active
  caller-workflow-bound path only. No first-delivery target uses this topology
  after npmjs was moved to the entry-hosted path. A successor design that enables
  this topology must identify the direct caller workflow filename configured in
  the registry, grant `id-token: write` only to jobs in that active caller chain
  that must pass the OIDC capability onward, and grant the child reusable publish
  job that requests the OIDC token. This is not a workflow-wide grant; unrelated
  matrix entries and unrelated jobs must not receive it.
- `external-oidc-reusable-workflow`: grant `id-token: write` only along the
  ...
  caller chain in current scope: the `official.yml` caller job invokes
...
| NuGet.org trusted publishing    | Each live-enabled `nuget/nuget-org` official target. The NuGet smoke official descriptor is in current descriptor/code scope, but live full-success is blocked until NuGet.org trusted publishing and service-side enablement are configured. Real package publication such as `hjg-pngcs` remains separately deferred until explicitly brought into scope. | Package owner-side trusted publisher for repository `hcoona/three`, conservative entry workflow filename `official.yml` with no `.github/workflows/` path, and environment `release`. External setup review and live evidence are required before treating an enabled NuGet.org target as live-publication ready; the missing registry-side setup blocks live success rather than making `nuget/nuget-org` globally deferred. |
...
Examples include
`npm/npmjs#hcoona-release-smoke-npm-dual#hcoona-release-smoke-npm-dual`,
`rubygems/rubygems-org#asciidoctor-latexmath#asciidoctor-latexmath`, and
`nuget/nuget-org#hcoona-release-smoke-nuget#Hcoona.ReleaseSmoke.Nuget`. A
missing or empty value enables none. First delivery does not define a wildcard
token; enabling a whole external registry target would be too coarse because
...
| PyPI release smoke package                | `hcoona-release-smoke-pypi`                         | `src/public/lib/hcoona-release-smoke-pypi/`                         | `pyproject.toml`                                               | `buddy`: GitHub Release with wheel and sdist. `official`: GitHub Release plus PyPI with wheel and sdist, using normal build-system NBGV and entry-workflow PyPI topology.                                                                                                        |
...
| NuGet.org release smoke package           | `hcoona-release-smoke-nuget`                        | `src/public/lib/hcoona-release-smoke-nuget/`                        | `hcoona-release-smoke-nuget.csproj`                            | `buddy`: GitHub Release with `.nupkg` and `.snupkg` plus GitHub Packages NuGet `.nupkg` using `Hcoona.ReleaseSmoke.Nuget`. `official`: GitHub Release plus NuGet.org with `.nupkg`; live `.snupkg` registry publication remains deferred.                                        |
...
| GitHub Packages release smoke package     | `hcoona-release-smoke-github-packages`              | `src/public/lib/hcoona-release-smoke-github-packages/`              | `hcoona-release-smoke-github-packages.csproj`                  | `buddy` and `official`: GitHub Release with `.nupkg` and `.snupkg` plus GitHub Packages NuGet `.nupkg`, proving the GitHub-token package topology under the same package identity policy.                                                                                        |
...
| GitHub Packages NuGet smoke publication and token isolation                             | `src/public/lib/hcoona-release-smoke-github-packages/`                                                                                                                                                                         | Buddy and official descriptor targets include `nuget/github-packages`; target-instance catalog marks GitHub-token topology and `profile-coexistence-rule: same-name-allowed` for GitHub Packages smoke identities                                                                                                   | Buddy and official plans snapshot target instance, `resolved-publish-identity`, and GitHub Packages disposition for the same NuGet package identity                                                                                                                                                                                                                       | Publish selector routes the logical `publish-node-id` only to the planned GitHub Packages publish path for both profiles                                                                                      | Build receipt for `.nupkg`; publish request/result record GitHub Packages package/version identity, target surface, artifact references, and registry result reference                                                                                                                                                                                                                                                                    | GitHub Packages observation matches package/version; no PyPI, npmjs, RubyGems.org, or external OIDC readiness evidence is required; workflow/job evidence proves `packages: write` is scoped only to the matching publish job and no external OIDC route is granted                                                                                                                                                                                                                                                                                                                               | GitHub-token job can mutate only its target surface; unrelated jobs remain without package-write or id-token elevation                                                                                                                          |
...
| Dual npmjs entry-hosted trusted publication                                             | `src/public/lib/hcoona-release-smoke-npm-dual/`                                                                                                                                                                                | Descriptor target `npm/npmjs` uses the unscoped official package while the buddy profile also targets scoped GitHub Packages npm; catalog requires first-delivery entry-workflow-bound OIDC topology because npm validates the calling workflow name for `workflow_call` publishes                                  | Plan snapshots npmjs target instance, `publish-topology`, target capabilities, `resolved-publish-identity`, and resolved unscoped package identity without confusing the scoped GitHub Packages buddy identity                                                                                                                                                            | `execution-sets.json` selects the npmjs node in the entry-workflow publish set and leaves reusable-publish and caller-workflow-bound sets empty for that node                                                 | npm pack receipt; publish request/result records npm package/version identity, tarball artifact reference, target instance, and registry result reference                                                                                                                                                                                                                                                                                 | npmjs registry/readiness workflow evidence proves the trusted publisher is configured for workflow filename `official.yml`, environment `release`, and package `hcoona-release-smoke-npm-dual`; the OIDC-token-requesting `npm publish` job is hosted in the top-level official entry workflow; no npmjs OIDC grant is present on `release-orchestrate.yml` or `release-publish-node.yml`                                                                                                                                                                                                 | Official run succeeds or skip-satisfies with no unrelated OIDC-capable jobs; reruns validate the dual smoke project rather than publishing real Hexo                                                                                            |
...
| GitHub Release asset content equivalence                                                | Same-tag GitHub Release fixture whose remote release has the expected state, asset names, and labels, but at least one asset has a missing, unverifiable, or mismatched GitHub Artifact Attestation-backed content proof       | Descriptor models GitHub Release assets with stable planned artifact identities                                                                                                                                                                                                                                     | Planner observation refuses `exact-satisfied` when any planned asset lacks matching attestation-backed digest/size evidence; plan freezes the full signer workflow identity as `attestation.signer-workflow`; same-name assets with mismatched or unavailable content proof enter the same-tag partial/conflict path or fail closed rather than becoming `skip-satisfied` | Selectors are emitted only when the replay matrix authorizes a publish mode; no selector is emitted for a false satisfied state                                                                               | Successful GitHub Release publish emits `github-release-asset-proof.json` and a GitHub Artifact Attestation for each uploaded asset; no skip receipt claims satisfaction for same-name assets whose bytes are unproved or mismatched                                                                                                                                                                                                      | GitHub Release observation downloads the remote asset, verifies its GitHub Artifact Attestation against the frozen full signer workflow identity `hcoona/three/.github/workflows/release-publish-node.yml` and selected commit, compares digest and size, and proves mismatched content is not accepted as already satisfied even when names and labels match                                                                                                                                                                                                                                     | Rerun skips only when every planned GitHub Release asset is content-equivalent; mismatched or unverifiable content fails closed or enters an authorized replacement path before any publish mutation                                            |
...
| Dispatch SHA pinning                                                                    | Workflow fixture dispatching by branch, lightweight tag, and annotated tag while moving the original ref after entry resolution                                                                                                | Entry inputs preserve the raw branch or tag and the once-resolved peeled commit SHA; annotated-tag evidence proves the tag object is peeled to the target commit                                                                                                                                                    | Planner request and plan use the resolved `commit-sha` only; all project discovery, version authority, target snapshots, release tags, and publish identities are derived from that commit                                                                                                                                                                                | Build, tag, publish, and report selectors all carry or reference the same resolved commit SHA rather than the raw ref                                                                                         | Build receipts, `tag-result.json`, publish requests/results, skip receipts, immutable proof wrappers, and diagnostics all record or validate the same resolved commit SHA; no later job checks out or consumes a moved branch/tag ref                                                                                                                                                                                                     | Git, tag, GitHub Release, package registry, and proof evidence correspond to the resolved commit; registry observations are not used to follow the moved dispatch ref                                                                                                                                                                                                                                                                                                                                                                                                                             | Workflow succeeds or fails solely against the resolved commit; moving the branch or tag after dispatch cannot change planned, built, tagged, or published content                                                                               |
| Same-entry workflow-level concurrency serialization                                     | Workflow fixture dispatching three same-entry runs with different raw refs, resolved commits, requested project subsets, or other entry inputs on the same entry workflow                                                      | Inputs preserve the differing raw refs, resolved commits, subsets, or other inputs while identifying the same top-level entry workflow for each evidence axis                                                                                                                                                       | Runs stay pinned to their own resolved `commit-sha`; no supersession diagnostic or replacement plan is emitted; raw ref, resolved commit, requested subset, and other input differences do not change current-scope concurrency identity                                                                                                                                  | Not applicable: concurrency is workflow-level evidence, not execution-set selector evidence; selectors add no extra selector proof beyond the resolved commit already carried by the dispatch SHA pinning row | No additional request/result/receipt evidence is required for concurrency; receipts remain ordinary positive evidence when their jobs run and are not used to prove the concurrency key; the replaced pending run has no positive release receipt                                                                                                                                                                                         | Workflow evidence shows each entry workflow declares exactly one workflow-level group, `three-release:buddy` or `three-release:official`, with `cancel-in-progress: false`; proves child reusable workflows, matrix rows, publish jobs, and report jobs do not declare that group; proves GitHub's one-running/one-pending behavior may cancel the older pending same-entry run when the third run queues; and proves no overlapping tag, publish request, entry-hosted OIDC token mint, report artifact upload, or live side effect for the same entry workflow          | The running same-entry workflow is not auto-cancelled; at most one pending same-entry workflow is retained by GitHub; older pending runs may be cancelled before side effects; matrix siblings are not canceled by same-group pending semantics |
