...
Current supersession note: logical Workflow Delivery v3 commit 11 retires both
legacy v1 Buddy entry files, `.github/workflows/buddy.yml` and
`.github/workflows/release-buddy.yml`, with no replacement
`legacy-buddy.yml`, dispatch, or caller-compatibility route. The v1
`.github/workflows/official.yml` and `.github/workflows/ci.yml` entries remain
active. Historical Buddy design notes below are retained only when explicitly
marked as superseded, legacy, or historical.
...
| Entry authorization       | Explicitly check active `official` as `maintain+` before planning; retired legacy Buddy entry routes have no dispatch or caller-compatibility path.                                                                                                                                                                                        |
...
| Dispatch SHA pinning      | Pin the release commit before orchestration: empty active Official `target` uses the GitHub UI dispatch ref/commit, while non-empty targets resolve once before orchestration; later stages stay pinned to that release SHA while workflow code remains from the trusted dispatch ref.                                                       |
...
- `buddy` and `official` remain the current-scope planner profiles, but only
  `official` remains an active v1 operator entry point after commit 11. The
  retired legacy Buddy entry files have no compatibility route; generic Buddy
  profile behavior is retained for planner-domain and v3 workflow coverage.
  `official` is a `maintain+` repository-maintenance path with
  protected-environment approval for live side effects.
...
The retired legacy `.github/workflows/buddy.yml` and
`.github/workflows/release-buddy.yml` files no longer participate in the active
topology and have no compatibility caller route. `release-resolve.yml`,
...
workflow filename for `workflow_call`. The retired legacy `buddy.yml` caller must not be configured as a current npmjs
trusted publisher; adding a future Buddy live npmjs caller would require a new
approved topology rather than a compatibility route.
...
The active Official entry workflow exposes the current operator-facing manual
dispatch input shape. The operator-facing `project` input is normalized into the
...
active `official.yml` dispatch surface; the retired legacy `buddy.yml` surface
is removed and has no compatibility route.
...
| `force_update_tag` | boolean | entry workflow | Operator-facing force input normalized to `request-flags.force`. For active `official`, it authorizes only reviewed release-tag retargeting. Legacy Buddy force behavior is historical only after the retired `buddy.yml` entry removal. Default is `false`. |
...
and pins to the resulting commit for the rest of the run. The former legacy
`buddy.yml` target-reachability entry rule is historical only after commit 11;
there is no current Buddy entry route that can use
...
The active `official.yml` entry workflow resolves raw dispatch target/ref
selectors before its concurrency-controlled orchestration job starts. It passes
the pinned `release_target_sha` to
...
is reusable behind active `official` and custom allowlisted callers; the retired
legacy `buddy.yml` caller has no compatibility route. It authenticates the OIDC
reusable-workflow caller claim and requires `official` to originate from
`official.yml` at the selected workflow ref. Custom allowlisted channels remain
...
Direct reusable callers are outside the active `official` entry contract.
Legacy `buddy` is reserved and rejected at this v1 reusable boundary. If an
allowlisted custom channel invokes `release-orchestrate.yml` directly,
orchestration delegates to `release-resolve.yml` to resolve and pin the provided
selector before downstream jobs run; the active Official entry workflow instead
performs that resolution first and passes the pinned `release_target_sha`.
...
3. Resolve `target` to the exact commit used for planning, build, tag, and
   publish work; an empty `target` means the selected dispatch commit. The
   former v1 Buddy reachability rule is historical after commit 11; generic
   Buddy planner semantics remain only for preserved domain tests and v3.
4. Preserve `force_update_tag` as the only active operator-facing force-style
   ...
   flag authorizes only reviewed release-tag retargeting. The former v1 Buddy
   mutable-prerelease behavior is historical at this entry boundary; its generic
   planner-domain invariants remain preserved without an active legacy caller.
   The flag is not a dry-run, validation-build, immutable-target bypass,
   identity-check bypass, or descriptor/catalog policy bypass.
...
6. For `official`, reject the run before planner execution unless the selected
   ...
   the selected project. The retired v1 Buddy entry cannot use or bypass this
   guard because it has no active caller route.
...
`authorize-entry` is the active Official control-plane gate and runs before
planner execution.
...
The former v1 Buddy `write` threshold is historical after commit 11. v3 Buddy
authorization is governed by the separate v3 workflows and policy.
...
- checked-in workflow expression: `${{ needs.authorize-entry.outputs.release_group }}` for the `orchestrate` job in active `official.yml`; legacy `buddy.yml` used the same expression before retirement.
...
release's active Official orchestration stays serialized by `official.yml`.
The retired legacy `buddy.yml` no longer contributes a concurrent entry route.
The lock starts after `authorize-entry`; pre-orchestration
...
The active `official.yml` dispatch surface does not expose
...
1. `official.yml` performs entry authorization and invokes
   `release-orchestrate.yml` with tag-derived or manual release controls. The
   legacy `buddy.yml` and `release-buddy.yml` entries are retired and have no
   compatibility route.
...
| Entry actor authorization fails closed                                  | Workflow fixture dispatching the active `official` entry with unresolved actor permission and with permission below the `maintain+` threshold; retired legacy Buddy old-ref or deleted-route dispatch attempts have no current route and are covered by the trusted-caller negative evidence | Inputs identify active Official profile and current-attempt actor; descriptors are irrelevant because rejection precedes discovery; retired Buddy route attempts are rejected before reusable policy or planning                                                                                                      | No planner request or plan is emitted; diagnostics record `REQ_ACTOR_UNAUTHORIZED` for unresolved and insufficient Official permission; retired Buddy old-ref/caller attempts emit only no-route or caller-identity failure evidence                                                                                                                                                                                                                                                                                                                                                                           | No `execution-sets.json` artifact is published; report shows no execution-set selectors, publish jobs, topology routes, build variants, or live routes were materialized                                                              | No `build-result.json`, `tag-result.json`, `publish-request.json`, `publish-result.json`, `skip-result.json`, or `release-immutable-proof-v1-...` artifact containing `immutable-proof.json` is produced                                                                                                                                                                                                                                                                         | Readiness proves failure before protected-environment approval, environment-bound live jobs, credentials, write-token use, OIDC token minting, registry mutation, or other live side effect; reusable caller identity evidence proves retired Buddy cannot be resurrected by direct or renamed callers                                                                                                                                                                                                                                                              | Unauthorized or unresolved Official actors fail before planning; authorized Official actors proceed without treating environment approval as authorization; retired legacy Buddy old refs and callers fail closed before planning                                                                 |
| Invalid entry input rejection                                           | Workflow fixtures dispatching the active `official` entry with missing `project`, invalid `version`, invalid `target`, or unsupported legacy inputs such as `dry-run`, `validation-build`, `force`, or `canary-override-non-public-ref`; retired legacy Buddy old-ref dispatch attempts are no-route/old-ref rejection evidence | Shared active Official input normalization records the rejected raw combination before planner execution; retired Buddy exact filenames are absent and any direct reusable `channel=buddy` caller fails the caller-identity guard rather than falling through as custom input | No planner request or plan is emitted; diagnostics identify `REQ_INVALID_INPUT` for invalid active Official inputs or unsupported legacy input names; retired Buddy direct/renamed callers emit caller-identity failure with reserved-channel diagnostics                                                                                                                                                                                                                                                                                                                                                       | No `execution-sets.json` artifact is published; report shows no execution-set selectors, publish jobs, topology routes, build variants, or live routes were materialized                                                              | No `build-result.json`, `tag-result.json`, `publish-request.json`, `publish-result.json`, `skip-result.json`, or `release-immutable-proof-v1-...` artifact containing `immutable-proof.json` is produced                                                                                                                                                                                                                                                                         | Readiness proves rejection before protected-environment approval, environment-bound live jobs, credentials, write-token use, OIDC token minting, registry mutation, tag creation, or other side effect; reserved `official`/`buddy` channel names cannot be supplied through `channel_allowlist` to bypass caller policy                                                                                                                                                                                                                                           | Invalid Official inputs fail with diagnostics, no partial plan, and no side effects; valid active inputs `project`, `version`, optional `target`, and `force_update_tag` continue; retired Buddy routes remain unavailable                                                                       |
| Trusted workflow-ref and reusable caller identity gate                  | Workflow-level fixture dispatching from default branch, protected release ref, unprotected branch, indeterminate protection state, and reusable calls from active `official.yml`, retired legacy `buddy.yml`, or an unexpected workflow        | Entry inputs identify the selected workflow ref and trusted-ref policy source; reusable orchestrator OIDC claims identify the caller `workflow_ref`, reusable `job_workflow_ref`, repository, and selected ref; CODEOWNERS wildcard coverage governs every checked-in workflow file capable of becoming a caller    | Trusted refs and exact active callers may reach normal planning; untrusted or indeterminate refs emit diagnostics-only `planner-diagnostics.json` with registered code `REQ_UNTRUSTED_WORKFLOW_REF`; retired legacy Buddy callers have no compatibility route; mismatched reusable callers fail before policy/resolve with no planner request or plan artifact                                                                                                                                                                                                                                                | Rejected refs or callers publish no `execution-sets.json` artifact; diagnostics or report evidence shows no execution-set selectors, publish jobs, topology routes, build variants, or live routes were materialized                  | Rejected refs or callers produce no `build-result.json`, `tag-result.json`, `publish-request.json`, `publish-result.json`, `skip-result.json`, or `release-immutable-proof-v1-...` artifact containing `immutable-proof.json`                                                                                                                                                                                                                                                    | Readiness evidence proves the trusted-ref and reusable-caller gates fail closed before planning, credentials, protected environment use, registry OIDC token minting, or any live side effect; custom allowlisted callers are governed by workflow CODEOWNERS coverage and cannot impersonate reserved `official` or `buddy` channels                                                                                                                                                                                                                            | Trusted refs and exact active callers proceed to the applicable scenario; untrusted refs, indeterminate refs, retired legacy Buddy callers, and mismatched reusable callers fail closed before planning                                                                                      |
| Dispatch SHA pinning                                                    | Official manual-dispatch fixtures cover empty `target` from the GitHub UI dispatch ref/commit plus non-empty branch, lightweight tag, annotated tag, ref, and 40-hex SHA selectors while moving the original target ref after entry resolution | Entry inputs preserve the raw selector when present and the once-resolved peeled release commit SHA; annotated-tag evidence proves the tag object is peeled to the target commit, while empty `target` evidence records the UI dispatch commit                                                                      | Planner request and plan use only the pinned release `commit-sha`; all project discovery, version authority, target snapshots, release tags, and publish identities derive from that immutable commit                                                                                                                                                                                                                                                                                                                                                                                                         | Build, tag, publish, and report selectors all carry or reference the same pinned release commit SHA rather than the raw selector                                                                                                      | Build receipts, `tag-result.json`, publish requests/results, skip receipts, immutable proof wrappers, and diagnostics all record or validate the same pinned release commit SHA; no later job checks out or consumes a moved target ref                                                                                                                                                                                                                                          | Git, tag, GitHub Release, package registry, and proof evidence correspond to the pinned release commit; registry observations do not follow the moved target ref, and workflow code remains from the trusted dispatch ref                                                                                                                                                                                                                                                                                                                                        | Workflow succeeds or fails solely against the pinned release commit; moving the target branch or tag after dispatch cannot change planned, built, tagged, or published content                                                                                                               |
| Same-release entry release-identity concurrency serialization           | Workflow fixture dispatching same-release Official runs with different raw refs or equivalent selectors that resolve to the same project/version release tag; the legacy `buddy.yml` half is retired and has no compatibility route            | `authorize-entry` resolves each raw selector before the lock and emits `needs.authorize-entry.outputs.release_group` for the `orchestrate` caller job                                                                                                                                                               | Runs stay pinned to their own resolved `commit-sha`; no supersession diagnostic or replacement plan is emitted; raw ref differences do not split the active dynamic job-level group after `authorize-entry`; entry authorization and the top-level pre-orchestration report bridge are not claimed as covered by the lock                                                                                                                                                                                                                                                                                     | Not applicable: concurrency is job-level workflow evidence, not execution-set selector evidence; selectors add no extra selector proof beyond the resolved target SHA carried into orchestration                                      | No additional request/result/receipt evidence is required for concurrency; receipts remain ordinary positive evidence when their jobs run and are not used to prove the concurrency key; the replaced pending orchestrate job has no positive release receipt                                                                                                                                                                                                                    | Workflow evidence shows active `official.yml` declares `concurrency.group: ${{ needs.authorize-entry.outputs.release_group }}` with `cancel-in-progress: false`; retired legacy `buddy.yml` contributes no active route; child reusable workflows, matrix rows, publish jobs, and top-level pre-orchestration report jobs do not declare those groups; proves GitHub's one-running/one-pending job behavior may cancel the older pending same-group orchestrate job before live side effects                                                                     | The running same-group orchestrate job is not auto-cancelled; at most one pending same-group orchestrate job is retained by GitHub; older pending orchestrate jobs may be cancelled before side effects; matrix siblings are not canceled by same-group pending semantics                    |
