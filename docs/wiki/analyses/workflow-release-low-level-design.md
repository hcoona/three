...
| NuGet.org release smoke package           | `hcoona-release-smoke-nuget`                        | `src/public/lib/hcoona-release-smoke-nuget/`                        | `hcoona-release-smoke-nuget.csproj`                            | `buddy`: GitHub Release with `.nupkg` and `.snupkg`. `official`: GitHub Release plus NuGet.org with `.nupkg`; live `.snupkg` registry publication remains deferred.                                                                             |
| npmjs release smoke package               | `hcoona-release-smoke-npm`                          | `src/public/lib/hcoona-release-smoke-npm/`                          | `package.json`                                                 | `buddy`: GitHub Release with the npm package tarball. `official`: GitHub Release plus npmjs trusted publishing with the same tarball.                                                                                                           |
| RubyGems.org release smoke package        | `hcoona-release-smoke-rubygems`                     | `src/public/lib/hcoona-release-smoke-rubygems/`                     | `hcoona-release-smoke-rubygems.gemspec`                        | `buddy`: GitHub Release with the `.gem`. `official`: GitHub Release plus RubyGems.org trusted publishing with the `.gem`.                                                                                                                       |
| GitHub Packages release smoke package     | `hcoona-release-smoke-github-packages`              | `src/public/lib/hcoona-release-smoke-github-packages/`              | `hcoona-release-smoke-github-packages.csproj`                  | `buddy`: GitHub Release with `.nupkg` and `.snupkg` evidence only. `official`: GitHub Release plus GitHub Packages NuGet `.nupkg`, proving the GitHub-token package topology against a smoke package behind the protected official environment. |
...
| GitHub Packages NuGet official publication and token isolation        | `src/public/lib/hcoona-release-smoke-github-packages/`                                                                                                                                                                         | Official descriptor target `nuget/github-packages`; buddy descriptor target is limited to `github-release/public`; target-instance catalog marks GitHub-token topology                                                                                                                                              | Official plan snapshots target instance, `resolved-publish-identity`, and GitHub Packages disposition; buddy plan has only GitHub Release evidence for this smoke package                                                                                                                                                                                                 | Official publish selector routes the logical `publish-node-id` only to the planned GitHub Packages publish path; buddy selector has no GitHub Packages publish node for the same package identity             | Official build receipt for `.nupkg`; publish request/result record GitHub Packages package/version identity, target surface, artifact references, and registry result reference                                                                                                                                                                                                                                                           | GitHub Packages observation matches official package/version; no PyPI, npmjs, RubyGems.org, or external OIDC readiness evidence is required; workflow/job evidence proves `packages: write` is scoped only to the matching official publish job and no external OIDC route is granted; buddy cannot publish the same immutable GitHub Packages identity                                                                                                                                                                                                                                           | Official GitHub-token job can mutate only its target surface behind the protected `release` environment; buddy and unrelated jobs remain without GitHub Packages package-write or id-token elevation                                            |
