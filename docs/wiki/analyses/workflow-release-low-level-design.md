...
| `.github/workflows/release-official.yml`      | `workflow_dispatch`   | `official` entry workflow, including early actor-permission authorization. | Trusted-publisher filename for first-delivery entry-hosted external registries.               |
...
`release-official.yml` for entry-hosted external publication and
...
npmjs as entry-hosted: configure npmjs to trust `release-official.yml` with the
...
- `release-buddy.yml`: `three-release:release-buddy.yml`;
- `release-official.yml`: `three-release:release-official.yml`.
...
in `.github/workflows/release-official.yml` with the GitHub Actions environment
...
- The concrete first-delivery `npm publish` command and OIDC token request must
  run in the top-level `release-official.yml` entry workflow under the
...
- The projected `npm/github-packages` path for `hexo-renderer-asciidoc` remains
  out of first-delivery live scope because its canonical npmjs tarball is
  unscoped while GitHub Packages npm requires an owner-scoped package identity.
  The release-smoke npm package instead uses the scoped identity
  `@hcoona/hcoona-release-smoke-npm` for both npmjs and GitHub Packages so buddy
  and official reuse the same package identity.
...
Examples include
`npm/npmjs#hcoona-release-smoke-npm#@hcoona/hcoona-release-smoke-npm`,
...
| Node npm package                          | `hexo-renderer-asciidoc`                            | `src/public/lib/hexo-renderer-asciidoc/`                            | `package.json`                                                 | `buddy`: GitHub Release with the npm package tarball. `official`: GitHub Release with the npm package tarball. Live npmjs acceptance now uses `hcoona-release-smoke-npm`.                                                                 |
