...
Current scope does not adopt native duplicate-run auto-cancellation. Each
...
all runs of the same entry workflow from run creation through final completion, so
the protected window naturally covers commit resolution, shared orchestration,
entry-hosted publish jobs, and final reporting without requiring a post-resolution
job-level lock. Cancellation therefore remains manual operator cancellation or
ordinary platform cancellation, not a repo-defined supersession protocol.
...
3. verify the downloaded file with `gh attestation verify` against repository
   ...
   planner-frozen signer workflow for the publish node, and the selected
   `commit-sha` as source digest;
...
- Normalize the asset set by asset name, label, digest, and size, then compare it
  ...
  Attestation-backed content proof for each planned `artifact-id`.
...
| Same-entry workflow-level concurrency serialization         | Workflow fixture dispatching paired same-entry evidence with different raw refs, resolved commits, requested project subsets, or other entry inputs on the same entry workflow                                                 | Inputs preserve the differing raw refs, resolved commits, subsets, or other inputs while identifying the same top-level entry workflow for each evidence axis                                                                                                                                                       | Runs stay pinned to their own resolved `commit-sha`; no supersession diagnostic or replacement plan is emitted; raw ref, resolved commit, requested subset, and other input differences do not change current-scope concurrency identity                                                                                                | Not applicable: concurrency is workflow-level evidence, not execution-set selector evidence; selectors add no extra selector proof beyond the resolved commit already carried by the dispatch SHA pinning row | No additional request/result/receipt evidence is required for concurrency; receipts remain ordinary positive evidence when their jobs run and are not used to prove the concurrency key                                                                                                                                                                                                                                                   | Workflow evidence shows each entry workflow declares exactly one workflow-level group, `three-release:release-buddy.yml` or `three-release:release-official.yml`, with `cancel-in-progress: false`; proves child reusable workflows, matrix rows, publish jobs, and report jobs do not declare that group; and proves no overlapping tag, publish request, entry-hosted OIDC token mint, report artifact upload, or live side effect for the same entry workflow                                                                                                                                  | Runs serialize by entry workflow from run creation until final completion; later runs wait rather than auto-cancelling earlier runs, and matrix siblings are not canceled by same-group pending semantics  |
