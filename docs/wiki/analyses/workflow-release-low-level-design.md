...
- `buddy` and `official` must not publish the same package name to the same
  ...
  marked `same-name-allowed`. That exception is limited to GitHub Packages
  GITHUB_TOKEN smoke validation for the NuGet, npm, RubyGems, and dedicated
  GitHub Packages smoke identities; it does not relax the separation required
  for external public registries.
