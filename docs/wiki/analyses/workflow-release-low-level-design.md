...
same-commit release continuation from overlapping live side effects. A coarser
entry-workflow concurrency key based only on the raw ref may be added for
operator convenience, but it is not a substitute for the resolved-SHA key
required above.
...
- Normalize the asset set by asset name, label, digest, and size, then compare it
  ...
  `projection.asset-labels-by-artifact-id` maps plus admissible build receipt or
  immutable-proof content evidence for each planned `artifact-id`.
...
| GitHub Release asset content equivalence                    | Same-tag GitHub Release fixture whose remote release has the expected state, asset names, and labels, but at least one asset's remote digest or size differs from admissible build receipt or proof evidence                                    | Descriptor models GitHub Release assets with stable planned artifact identities                                                                                                                                                                                                                                     | Planner observation refuses `exact-satisfied` when any planned asset lacks matching remote digest/size evidence; same-name assets with mismatched or unavailable content proof enter the same-tag partial/conflict path or fail closed rather than becoming `skip-satisfied`                                                            | Selectors are emitted only when the replay matrix authorizes a publish mode; no selector is emitted for a false satisfied state                                                                           | Build receipt or immutable-proof evidence supplies the planned digest and size for each asset; no skip receipt claims satisfaction for same-name assets whose bytes are unproved or mismatched                                                                                                                                                                                                        | GitHub Release observation includes remote asset name, label, digest, and size evidence, and proves mismatched content is not accepted as already satisfied even when names and labels match                                                                                                                                                                                                                                                                                                                                                                                                      | Rerun skips only when every planned GitHub Release asset is content-equivalent; mismatched content fails closed or enters an authorized replacement path before any publish mutation                                                                    |
