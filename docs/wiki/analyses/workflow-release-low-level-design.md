...
`release-buddy.yml` or `release-official.yml`. That same group must cover the
shared orchestration workflow call and every side-effect-capable job that can run
after orchestration returns to the entry workflow, including entry-hosted publish
jobs and the final report job. The resolved-SHA lock therefore must not end at
the reusable orchestration call if the run still has entry-hosted publish or
report work remaining. A coarser entry-workflow concurrency key based only on the
raw ref may be added for operator convenience, but it is not a substitute for the
resolved-SHA key required above.
